...
var baseURL =
  ...
  "/bookingfrontend/";
$(".termAcceptDocsUrl").attr(
  "data-bind",
  "text: docName, attr: {'href': itemLink }"
);
$(".maleInput").attr(
  "data-bind",
  "textInput: inputCountMale, attr: {'name': malename }"
);
$(".femaleInput").attr(
  "data-bind",
  "textInput: inputCountFemale, attr: {'name': femalename }"
);
var urlParams = [];
...
function applicationModel() {
  ...
  self.isResourceSelected = ko
    .computed(function() {
      var k = 0;
      for (var i = 0; i < self.bookableResource().length; i++) {
        if (self.bookableResource()[i].selected()) {
          if (
            self.selectedResources.indexOf(self.bookableResource()[i].id) < 0
          ) {
          ...
          if (
            self.selectedResources.indexOf(self.bookableResource()[i].id) > -1
          ) {
            self.selectedResources.splice(
              self.selectedResources.indexOf(self.bookableResource()[i].id),
              1
            );
          }
        }
  ...
  self.addDate = function() {
    if (
      ...
      if (start.getTime() < end.getTime()) {
        ...
        $(".applicationSelectedDates").html(
          "Starttid m&aring; v&aelig;re tidligere enn sluttid"
        );
      }
    }
  };
  ...
  self.termAccept = ko.computed(function() {
    var notAccepted = ko.utils.arrayFirst(self.termAcceptDocs(), function(
      current
    ) {
    ...
    $.getJSON(getJsonURL, function(result) {
      for (var i = 0; i < result.results.length; i++) {
        if (result.results[i].building_id == urlParams["building_id"]) {
          ...
          if (
            typeof urlParams["resource_id"] !== "undefined" &&
...
function PopulatePostedDate() {
  if (initialDates != null) {
    ...
    if (
      typeof urlParams["start"] !== "undefined" &&
      typeof urlParams["end"] !== "undefined"
    ) {
      if (urlParams["start"].length > 0 && urlParams["end"].length > 0) {
        am.date.push({
          from_: formatSingleDate(new Date(parseInt(urlParams["start"]))),
          to_: formatSingleDate(new Date(parseInt(urlParams["end"]))),
          /*repeat: false,*/ formatedPeriode: formatDate(
            new Date(parseInt(urlParams["start"])),
            new Date(parseInt(urlParams["end"]))
          )
        });
      }
    }
...
var dateformat_datepicker = dateformat_backend
  .replace(/d/gi, "%d")
  .replace(/m/gi, "%m")
  .replace(/y/gi, "%Y");
...
YUI({ lang: "nb-no" }).use("aui-datepicker", function(Y) {
  new Y.DatePicker({
...
YUI({ lang: "nb-no" }).use("aui-timepicker", function(Y) {
  new Y.TimePicker({
    ...
    mask: "kl. %H:%M",
    popoverCssClass:
      "timepicker-popover yui3-widget popover yui3-widget-positioned yui3-widget-modal yui3-widget-stacked bookingStartTime-popover",
    on: {
      selectionChange: function(event) {
        new Date(event.newSelection);
...
YUI({ lang: "nb-no" }).use("aui-timepicker", function(Y) {
  new Y.TimePicker({
    ...
    mask: "kl. %H:%M",
    popoverCssClass:
      "timepicker-popover yui3-widget popover yui3-widget-positioned yui3-widget-modal yui3-widget-stacked bookingEndTime-popover",
    on: {
      selectionChange: function(event) {
        new Date(event.newSelection);
        $(this).val(event.newSelection);
