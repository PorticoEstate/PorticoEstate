	...
	*/
	...
	{		
		...
		function get_status_for_check_list(){
			...
			if( $this->check_list->get_status() == 0 & $this->check_list->get_planned_date() > 0 & $this->check_list->get_deadline() > $todays_date_ts)
			{
				$status = "control_planned";
			}
			else if( $this->check_list->get_status() == 0 & $this->check_list->get_planned_date() > 0 & $this->check_list->get_deadline() < $todays_date_ts )
			{
				$status = "control_not_accomplished_with_info";
			}
			else if( $this->check_list->get_status() == 0 & $this->check_list->get_deadline() < $todays_date_ts )
			{
				$status = "control_not_accomplished";
			}
			else if( $this->check_list->get_status() == 1 & $this->check_list->get_completed_date() > $this->check_list->get_deadline() & $this->check_list->get_num_open_cases() == 0)
			{
			...
			else if( $this->check_list->get_status() == 1 & $this->check_list->get_num_open_cases() > 0){
