...
class rental_socontract extends rental_socommon
{
	...
	protected function get_query(string $sort_field, boolean $ascending, string $search_for, string $search_type, array $filters, boolean $return_count)
	{
		...
		else if ($sort_field == 'party')
		{
			...
			$this->order_field = 'party.company_name, party.last_name, party.first_name';
		}
		...
		if(isset($filters['contracts_for_billing']))
		{
			...
			$this->order_field = 'invoice.timestamp_end, contract.billing_start, contract.date_start, contract.date_end';
		...
		{
			// Sigurd 4. feb 2012: The improved query is 50 % faster - might have the use the old one for non-postgresql-db
			// $cols = 'COUNT(DISTINCT(contract.id)) AS count';
			// return "SELECT {$cols} FROM {$tables} {$joins} WHERE {$condition}";
			return "SELECT COUNT(id) AS count FROM (SELECT DISTINCT(contract.id)  as id FROM {$tables} {$joins} WHERE {$condition}) as t";
		}
		else
		{
			// columns to retrieve
	...
	public function get(int $start_index, int $num_of_objects, string $sort_field, boolean $ascending, string $search_for, string $search_type, array $filters)
	{
		...
		$sql_minimized = 'SELECT DISTINCT contract.id '  . substr($sql,strripos($sql,'FROM'));
	...
	//_debug_array($sql_end);
		...
		{
			$this->db->query("{$sql_parts[0]} contract.id = {$id}",__LINE__,__FILE__);
			while ($this->db->next_record())
			{
				$result = &$results[$id];
