<?php
	class booking_filestorage {
		...
		protected function check_storage_dir() {
			if (is_dir($this->get_files_path()) 
					&& is_writable($this->get_files_path()) 
					&& is_readable($this->get_files_path())
				) { return true; }
		...
		protected function create_storage_dir_if_not_exists() {
			if (is_dir($this->get_files_path())) { return false; }
	...
	class booking_storage_object {
		...
		protected function with_storage() {
			if (!$this->is_attached()) throw new booking_unattached_storage_object('Not attached to a storage');
			return $this->storage;
		}
