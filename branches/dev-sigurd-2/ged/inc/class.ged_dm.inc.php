...
class ged_dm
{
	...
	function move_element($element_id,$new_parent_id)
	{
		...
		if ( $element_info['project_root'] != $new_parent_info['project_root'] )
		{
			$this->set_project ($element_id, $new_parent_info['project_name'], $new_parent_info['project_root']);
		}
	}
	...
	// DONE acl management 
	function list_elements($parent_id=0, $type='', $order='name', $criteria='readable')
	{
		...
		while ($this->db->next_record())
		{
			...
			{
				if (($criteria == 'writable' && $this->can_write($the_element_id)) || $criteria == 'readable')
				{
					$go=true;	
				}
			}
	...
	{
		return( $this->list_elements($element_id, 'folder', 'name', $criteria));
	}
  ...
  function get_stats ($start_date=null, $end_date=null, $status=null, $project_root_id=null)
  {
	...
	function list_chronos($project_root_id)
	{
		...
		while($db2->next_record())
		{
			...
			$out[$doc_type][$i]['description']=$db2->f('description')." / ".$doc_version['description'];
	...
	function list_all_versions($project_root_id)
	{
		...
		$sql.="LEFT JOIN ged_doc_types ON ged_elements.doc_type = ged_doc_types.type_id ) ";
		$sql.="ORDER BY ged_elements.doc_type, ged_elements.creation_date";
