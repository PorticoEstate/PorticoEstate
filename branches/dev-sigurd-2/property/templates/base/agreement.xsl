	...
	<xsl:template match="add_activity">
		<table>
			...
			<tr>
				...
				<td>
					<textarea cols="60" disabled="disabled" rows="6" name="values[descr]" wrap="virtual" onMouseout="window.status='';return true;">
						<xsl:attribute name="onMouseover">
							<xsl:text>window.status='</xsl:text>
	...
	<xsl:template match="edit">
		...
		<table cellpadding="2" cellspacing="2" width="100%" align="center" border="0">
			...
			<tr >
				...
				<td valign="top">
					<xsl:variable name="lang_agreement_group_statustext"><xsl:value-of select="lang_agreement_group_statustext"/></xsl:variable>
					<select name="values[group_id]" class="forms" onMouseover="window.status='{$lang_agreement_group_statustext}'; return true;" onMouseout="window.status='';return true;">
						<option value=""><xsl:value-of select="lang_no_agreement_group"/></option>
					...
					<form method="post" name="form2" action="{$update_action}">
						...
						<table width="70%" cellpadding="2" cellspacing="2" >
							<!-- Buttons 2 -->
								...
								<img id="values_date-trigger" src="{img_cal}" class="calendar-opt" alt="{lang_datetitle}" title="{lang_datetitle}" style="cursor:pointer; cursor:hand;"  />
