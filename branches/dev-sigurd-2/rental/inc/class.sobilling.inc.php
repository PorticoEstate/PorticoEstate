...
class rental_sobilling extends rental_socommon
{
	...
	{
		/*
		var_dump($GLOBALS['phpgw_info']['user']['domain']);
		...
		*/
		...
		$this->db->query($sql_contracts, __LINE__, __FILE__,false,true);
		while($this->db->next_record())
		{
		        $contract_ids[] = $this->unmarshal($this->db->f('contract_id'), 'int');
		}
		//var_dump($contract_ids);
		foreach($contract_ids as $contract_id)
		{
		        ...
		        $this->db->query($sql_parties, __LINE__, __FILE__,false,true);
		        while($this->db->next_record())
		        {
