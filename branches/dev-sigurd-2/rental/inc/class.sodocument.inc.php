<?php
	...
	abstract class rental_sodocument extends rental_socommon
	{
		...
		{
			$this->ownerType = substr(get_class($this), 18);
			
			parent::__construct(sprintf('rental_document_%s', $this->get_owner_type()), 
				array(
					'id'			=> array('type' => 'int'),
					...
					'owner_name'	=> array(
						'type' => 'string',
						 'query' => true,
						 'join' => array(
							'table' => sprintf('rental_%s', $this->get_owner_type()),
							'fkey' => 'owner_id',
							'key' => 'id',
							'column' => 'name'
						)
			...
			$versionfile = $GLOBALS['phpgw']->common->get_app_dir('rental') . '/setup/setup.inc.php';
			include $versionfile;
			
			$this->uploadRootDir = $setup_info['rental']['document']['upload_dir'];
		}
		...
		{
			$parent_so = CreateObject(sprintf('rental.so%s', $this->get_owner_type()));
