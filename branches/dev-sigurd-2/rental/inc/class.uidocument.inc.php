<?php
	...
	abstract class rental_uidocument extends rental_uicommon
	{
		protected
		...
		protected function set_business_object(rental_bodocument $bo = null)
		{
		...
		{
			return CreateObject(sprintf('rental.bodocument_%s', $this->get_document_owner_type()));
		}
		...
		{
			return self::link(array('menuaction' => sprintf('rental.uidocument_%s.%s', $documentOwnerType, $action), 'filter_owner_id' => $documentOwnerId));
		}
				...
				'datatable' => array(
					...
					'field' => array(
						...
						array(
							'key' => 'actions',
							'label' => lang('Actions'),
							'formatter' => 'YAHOO.rental.'.sprintf('formatGenericLink(\'%s\', \'%s\')', lang('edit'), lang('delete')),
						),
						array(
							'key' => 'link',
							'hidden' => true
