...
class rental_agresso_gl07 implements rental_exportable
{
	...
	protected function run()
	{
		...
		foreach($invoices as $invoice) // Runs through all invoices
		{
			...
			$this->lines[] = $this->get_line($invoice->get_account_out(), $invoice->get_responsibility_id(), $invoice->get_service_id(), $buildine_location_code, $invoice->get_project_id(), '', $invoice->get_total_price(), $description, $invoice->get_contract_id(), $this->billing_job->get_year(), $this->billing_job->get_month());
		}
	}
