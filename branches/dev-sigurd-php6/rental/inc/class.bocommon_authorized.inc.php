<?php
	...
	phpgw::import_class('rental.unauthorized_exception');

	abstract class rental_bocommon_authorized extends rental_bocommon
	{
		...
		protected $defaultObjectPermissions = array(
			rental_sopermission::ROLE_DEFAULT =>
			array(
				'read' => true,
			),
		);
		...
		{
			$current_type = substr(get_class($this), 9);
			return sprintf('rental.sopermission_%s', $current_type);
		}
		...
		{	
			$default_role = array('role' => rental_sopermission::ROLE_DEFAULT);
		...
		protected function get_subject_global_roles() {
			...
			{
				$result = CreateObject('rental.sopermission_root')->read(array('filters' => array('subject_id' => $this->current_account_id())));
		 ...
		 * @throws rental_unauthorized_exception if not authorized
		 */
		protected function authorize($operation, $object = null)
		{
			...
			throw new rental_unauthorized_exception($operation, sprintf('Operation \'%s\' was denied on %s %s', $operation, get_class($this), is_null($object) ? 'collection' : 'object'));
		}
