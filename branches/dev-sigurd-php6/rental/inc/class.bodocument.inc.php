<?php
	phpgw::import_class('rental.bocommon_authorized');
	
	abstract class rental_bodocument extends rental_bocommon
	{
		protected 
			$owner_bo;
		...
		{
			parent::__construct();
			$owningType = substr(get_class($this), 18);
			$this->so = CreateObject(sprintf('rental.sodocument_%s', $owningType));

			// Add s to use plural form of class name (rentalcompositeS)
			$this->owner_bo = CreateObject(sprintf('rental.bo%s', $owningType));
		}
