<?php
	...
	abstract class rental_sopermission extends rental_socommon
	{
		...
		{
			$this->object_type = substr(get_class($this), 20);
			...
			if ($this->object_type == 'rentalcomposites') {
				$this->object_type = 'composite';
				$table_def['composite_id'] = $table_def['id'];
				unset($table_def['id']);
			}
			...
			parent::__construct('rental_permission', $table_def);
		...
		protected function build_object_relations()
		{
			return array(
				'object_name'	=> array(
					...
					'query' => true,
					'join' => array(
						'table' => sprintf('rental_%s', $this->get_object_type()),
						'fkey' => 'object_id',
						'key' => 'id',
						'column' => 'name'
					)
		...
		{
			$object_so = CreateObject(sprintf('rental.so%s', $this->get_object_type()));
