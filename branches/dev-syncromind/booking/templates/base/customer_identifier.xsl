...
<func:function name="phpgw:booking_customer_identifier_show" xmlns:php="http://php.net/xsl">
	...
	<func:result>
            ...
            <xsl:if test="$entity/customer_identifier_label">
                <span><xsl:value-of select="concat(php:function('lang', string($entity/customer_identifier_label)), ': ')"/><xsl:value-of select="$entity/customer_identifier_value"/></span>
            </xsl:if>
            <xsl:if test="not($entity/customer_identifier_label)">
                <span><xsl:value-of select="php:function('lang', 'None')"/></span>
            </xsl:if>
	</func:result>
</func:function>