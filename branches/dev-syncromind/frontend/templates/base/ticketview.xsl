				...
				<div id="{$tab_selected}">
					<ul style="margin: 2em;">
						<li style="margin-bottom: 1em;">
							<a href="{helpdesklist}"> &lt;&lt; 
								<xsl:value-of select="php:function('lang', 'show_all_tickets')"/>
							</a>
						</li>
						<li>
							<ul>
								...
								<li class="ticket_detail">
									<img src="frontend/templates/base/images/16x16/clock_edit.png" class="list_image"/>
									<xsl:value-of select="php:function('lang', 'entry_date')"/>
									<xsl:value-of select="ticket/entry_date"/>
									<xsl:value-of select="php:function('lang', 'of')"/>
									<xsl:value-of select="ticket/user_name"/>
								</li>
								<li class="ticket_detail">
									<img src="frontend/templates/base/images/16x16/timeline_marker.png" class="list_image"/>
									<xsl:value-of select="php:function('lang', 'status')"/>: <xsl:value-of select="ticket/status_name"/>
								</li>
								<xsl:choose>
									<xsl:when test="ticket/value_vendor_name">
										<li class="ticket_detail">
											<img src="frontend/templates/base/images/16x16/user_suit.png" class="list_image"/>
											<xsl:value-of select="php:function('lang', 'vendor')"/>: <xsl:value-of select="ticket/value_vendor_name"/>
										</li>
									</xsl:when>
								</xsl:choose>
								<xsl:choose>
									<xsl:when test="ticket/assigned_to_name">
										<li class="ticket_detail">
											...
											<xsl:value-of select="php:function('lang', 'assigned_to')"/>: <xsl:value-of select="ticket/assigned_to_name"/>
										</li>
									</xsl:when>
								</xsl:choose>
								<xsl:choose>
									<xsl:when test="ticket/value_contact_name">
										<li class="ticket_detail">
											<img src="frontend/templates/base/images/16x16/user_green.png" class="list_image"/>
											<xsl:value-of select="php:function('lang', 'contact')"/>: <xsl:value-of select="ticket/value_contact_name"/>
								...
								<xsl:choose>
									<xsl:when test="ticket/publish_note = 1">
										<li class="ticket_detail">
											...
											<xsl:value-of select="php:function('lang', 'message')"/>: <xsl:value-of select="ticket/details"/>
										</li>
									</xsl:when>
								</xsl:choose>
								<li class="ticket_detail">
