	...
	*/
	...
	class property_uiagreement extends phpgwapi_uicommon_jquery
	{
		...
		function index()
		{
            ...
            //echo '<pre>'; print_r($data); echo '</pre>'; exit('readd');
...
//			$GLOBALS['phpgw']->xslttpl->set_var('phpgw',array('add_activity' => $data));
            self::render_template_xsl(array('agreement','datatable_inline','attributes_form'), array('add_activity' => $data));
		}
        ...
        public function save()
        {
           ...
           if ((isset($values['save']) && $values['save']) || (isset($values['apply']) && $values['apply']))
           {
					...
					if(!$receipt['error'])
					{
                        ...
                        $GLOBALS['phpgw']->redirect_link('/index.php',array('menuaction'=>'property.uiagreement.edit','id'=>$id));
					}
                    else
                    {
                        $this->edit();
            ...
            if(!empty($type_alarm)){
                ...
                if ($type_alarm == 'update' || $type_alarm == 'update_item')
				{
                    ...
                    if($type_alarm == 'update'){
                        ...
                        $requestUrl	= json_encode(self::link(array('menuaction'=>'property.uiagreement.get_contentitem','agreement_id'=>$idAgreement, 'activity_id' =>$ids_alarm[0], 'phpgw_return_as'=>'json')));
				...
				else if(($type_alarm == 'delete_alarm' || $type_alarm == 'delete_item' ) && count($ids_alarm))
				{
                    ...
                    else
                    {
                        ...
                        $requestUrl	= json_encode(self::link(array('menuaction'=>'property.uiagreement.get_contentitem','agreement_id'=>$idAgreement, 'activity_id' =>$ids_alarm,'role'=> null, 'phpgw_return_as'=>'json')));
                        return $requestUrl;
                    }
				}
		...
		function edit($values = array(), $mode = 'edit')
		{
			...
			if(!$this->acl_add)
			{
				$GLOBALS['phpgw']->redirect_link('/index.php',array('menuaction'=> 'property.uilocation.stop', 'perm'=>$mode, 'acl_location'=> $this->acl_location));
			}
			...
			(
                array('my_name' => 'view', 'text' =>lang('View'),
                        'action'  => $GLOBALS['phpgw']->link('/index.php',array('menuaction' => 'property.uiagreement.view_item','agreement_id'=>$id)),'parameters'	=> json_encode($parameters)),
                array('my_name' => 'edit','text' =>lang('Edit'),
                        'action'  => $GLOBALS['phpgw']->link('/index.php',array('menuaction' => 'property.uiagreement.edit_item','agreement_id'=>$id)),'parameters'	=> json_encode($parameters)),
                array('my_name' => 'delete','text' =>lang('Delete'),
                        'action'  => $GLOBALS['phpgw']->link('/index.php',array('menuaction' => 'property.uiagreement.deleteitem', 'agreement_id'=>$id)),'parameters'	=> json_encode($parameters)),
				array('my_name'	=> 'select_all'),
				array('my_name'	=> 'select_none')
			);
            ...
            $myColumnDefs1 = array
            (
                array('key' => 'id', 'label' =>$table_header[0]['header'], 'sortable' => false, 'resizeable' => true),
                array('key' => 'num', 'label' =>$table_header[1]['header'], 'sortable' => false, 'resizeable' => true),
                array('key' => 'descr', 'label' =>$table_header[2]['header'], 'sortable' => false, 'resizeable' => true),
                array('key' => 'unit_name',	'label' =>$table_header[3]['header'], 'sortable' => false, 'resizeable' => true, 'formatter' => 'JqueryPortico.FormatterCenter'),
                array('key' => 'm_cost', 'label' =>$table_header[4]['header'], 'sortable' => false, 'resizeable' => true, 'formatter' => 'JqueryPortico.FormatterRight'),
                array('key' => 'w_cost', 'label' =>$table_header[5]['header'], 'sortable' => false, 'resizeable' => true, 'formatter' => 'JqueryPortico.FormatterRight'),
                array('key' => 'total_cost', 'label' =>$table_header[6]['header'], 'sortable' => false, 'resizeable' => true, 'formatter' => 'JqueryPortico.FormatterRight'),
                array('key' => 'this_index', 'label' =>$table_header[7]['header'], 'sortable' => false, 'resizeable' => true),
                array('key' => 'index_count', 'label' =>$table_header[8]['header'],	'sortable' => false, 'resizeable' => true, 'formatter' => 'JqueryPortico.FormatterCenter'),
                array('key' => 'index_date', 'label' =>$table_header[9]['header'], 'sortable' => false, 'resizeable' => true),
//                $permission_update?array('key' => 'select',	'label' =>$table_header[13]['header'], 'sortable' => false, 'resizeable' => false, 'formatter'=>FormatterCheckItems):"",
		...
		function edit_item()
		{
            ...
            $myColumnDefs0 = array
            (
                array('key' => 'activity_id','label'=>lang('Activity ID'),'sortable'=>false,'resizeable'=>true),
			...
			(
                ...
                'requestUrl'    => json_encode(self::link(array('menuaction'=>'property.uiagreement.get_contentitem','agreement_id'=>$agreement_id, 'activity_id' =>$id, 'phpgw_return_as'=>'json'))),
                'data'          => json_encode(array()),
				'ColumnDefs'	=> $myColumnDefs0,
				'config'		=> array(
					array('disableFilter'	=> true),
            ...
            self::add_javascript('property', 'portico', 'agreement.edit.js');
            self::render_template_xsl(array('agreement','datatable_inline','attributes_form'), array('edit_item' => $data));
		}
		...
		function view_item()
		{
            ...
            $myColumnDefs0 = array
            (
                array('key' => 'activity_id',	'label'=>lang($table_header[0]['header']),	'sortable'=>true,'resizeable'=>true ),
                array('key' => 'm_cost',		'label'=>lang($table_header[2]['header']),	'sortable'=>true,'resizeable'=>true, 'formatter'=>'JqueryPortico.FormatterRight' ),
                array('key' => 'w_cost',		'label'=>lang($table_header[3]['header']),	'sortable'=>true,'resizeable'=>true, 'formatter'=>'JqueryPortico.FormatterRight' ),
                array('key' => 'total_cost',	'label'=>lang($table_header[4]['header']),	'sortable'=>true,'resizeable'=>true, 'formatter'=>'JqueryPortico.FormatterRight' ),
                array('key' => 'this_index',	'label'=>lang($table_header[5]['header']),	'sortable'=>true,'resizeable'=>true ),
                array('key' => 'index_count',	'label'=>lang($table_header[6]['header']),	'sortable'=>true,'resizeable'=>true, 'formatter'=>'JqueryPortico.FormatterCenter' ),
                array('key' => 'index_date',	'label'=>lang($table_header[7]['header']),	'sortable'=>true,'resizeable'=>true )
            );
            ...
            phpgwapi_jquery::load_widget('core');
			phpgwapi_jquery::load_widget('numberformat');
            self::render_template_xsl(array('agreement','datatable_inline','attributes_view'), array('view_item' => $data));
		}
		...
		function view()
		{
			...
			$myColumnDefs = array
            (
                array('key' => 'time',	'label'=>$alarm_data['header'][0]['lang_time'],	'sortable'=>true,'resizeable'=>true),
                ...
                array('key' => 'enabled','label'=>$alarm_data['header'][0]['lang_enabled'], 'sortable'=>true,'resizeable'=>true,'formatter'=>'JqueryPortico.FormatterCenter'),
            );
			...
			$myColumnDefs1 = array
			(	
                array('key' => 'activity_id',	'label'=>$table_header[0]['header'], 'sortable'=>FALSE,'resizeable'=>true),
                array('key' => 'num',			'label'=>$table_header[1]['header'], 'sortable'=>FALSE,'resizeable'=>true),
                array('key' => 'descr',			'label'=>$table_header[2]['header'], 'sortable'=>FALSE,'resizeable'=>true),
                array('key' => 'unit_name',		'label'=>$table_header[3]['header'], 'sortable'=>FALSE,'resizeable'=>true, 'formatter'=>'JqueryPortico.FormatterCenter'),
                array('key' => 'm_cost',		'label'=>$table_header[4]['header'], 'sortable'=>FALSE,'resizeable'=>true, 'formatter'=>'JqueryPortico.FormatterRight'),
...
//					'myColumnDefs'				=> $myColumnDefs,
            ...
            self::render_template_xsl(array('agreement','datatable_inline','files','nextmatchs','attributes_view'), array('view' => $data));
		}
	}
