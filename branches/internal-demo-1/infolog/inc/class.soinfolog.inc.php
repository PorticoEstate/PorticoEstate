	...
	class soinfolog 				// DB-Layer
	{
		...
		*/
		function check_access($info_id,$required_rights)
		{
			...
			if(intval($info['info_responsible']) > 0 && $owner != $info['info_responsible'] && $owner != $this->user && $info['info_responsible'] == $this->user)
			{
				if($required_rights == 'confirm' && ($info['info_confirm'] == 'accept' || $info['info_confirm'] == 'both'))
				{
					$access_ok = ($info['info_confirm_status'] == 'not');
					return $access_ok;
				}

				if($required_rights == 'finish' && ($info['info_confirm'] == 'finish' || $info['info_confirm'] == 'both'))
				{
					$access_ok = ($info['info_confirm_status'] != 'finished' && $info['info_confirm_status'] != 'canceled');
					return $access_ok;
				}

				if($required_rights == 'cancel')
				{
					$access_ok = ($info['info_confirm_status'] != 'canceled');
					return $access_ok;
				}
				return False;
			}
			...
			//echo "check_access(info_id=$info_id (owner=$owner, user=" . $info['info_responsible'] . "),required_rights=$required_rights): access".($access_ok?"Ok":"Denied") . '<br />';
			return $access_ok;
		}
		...
		*/           
		function aclFilter($filter = 'none')
		{
			...
			if ($filter == 'own')
			{
				$filtermethod .= ' OR info_responsible=' . $this->user . " OR (info_status = 'offer' AND info_access='public')";
			}
			else
			{
		...
		*/
		function delete($info_id,$delete_children=True)  // did _not_ ensure ACL
		{
			...
			$this->db->query('DELETE FROM phpgw_infolog_extra WHERE info_id='.$info_id,__LINE__,__FILE__);
			...
			if ($delete_children)
			{
				...
				$db2->query('SELECT info_id FROM phpgw_infolog '
						. "WHERE info_id_parent=$info_id "
						. 'AND info_owner=' . $this->user,__LINE__,__FILE__);
		...
		*/
		function change_delete_owner($owner,$new_owner=0)  // new_owner=0 means delete
		{
			...
			if (!($new_owner = intval($new_owner)))
			{
				$db2 = $this->db;	// we need an extra result-set
				$db2->query('SELECT info_id FROM phpgw_infolog '
						. "WHERE info_owner=$owner",__LINE__,__FILE__);
				while($db2->next_record())
				{
			...
			$this->db->query('UPDATE phpgw_infolog '
					...
					. "WHERE info_responsible=$owner",__LINE__,__FILE__);
		}
		...
		*/
		function search($order,$sort,$filter,$cat_id,$query,$action,$action_id,$ordermethod,&$start,&$total)
		{
			...
			if ((!isset($GLOBALS['phpgw_info']['user']['preferences']['infolog']['listNoSubs']) || !$GLOBALS['phpgw_info']['user']['preferences']['infolog']['listNoSubs']) &&
				 $action != 'sp')
			{
				$pid = '';
			}
		...
		{
			$this->db->query("Update phpgw_infolog set info_confirm_status='$confirm_status' where info_id=" . intval($info_id),__LINE__,__FILE__);
		}
	}
?>
