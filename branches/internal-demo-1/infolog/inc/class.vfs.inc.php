 ...
 *License, or at your option) any later version.			*
 \**********************************************************************/
 /* $Id: class.vfs.inc.php 17949 2007-02-13 15:02:07Z sigurdne $ */
		...
		*/
		function cd ($target = '/', $relative = True, $relatives = '')
		{
			...
			if ($relative == 'relative' || $relative == True)
			{
				...
				{
					$currentdir = $GLOBALS['phpgw']->session->appsession($location = 'default', $appname = 'infolog');
			...
			$GLOBALS['phpgw']->session->appsession($location = 'default', $appname = 'infolog',$data = $basedir);
		...
		*/
		function file_type ($file, $relatives = '')
		{
			...
			*/
			...
			$GLOBALS['phpgw']->db->next_record ();
			$mime_type = isset($GLOBALS['phpgw']->db->Record['mime_type']) ? $GLOBALS['phpgw']->db->Record['mime_type'] : '';
		...
		*/
		function ls ($dir = False, $relatives = '', $checksubdirs = True, $mime_type = False, $nofiles = False, $orderby = 'directory')
		{
			...
			if (((($type = $this->file_type ($dir, array ($p->mask))) != 'Directory') || ($nofiles)) && !$p->outside)
			{
				...
				while (list ($num, $attribute) = each ($this->attributes))
				{
					$rarray[0][$attribute] = isset($record[$attribute]) ? $record[$attribute] : '';
				}
