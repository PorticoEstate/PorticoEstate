<!-- $Id$ -->
	...
	<xsl:template match="values">
		<xsl:variable name="lang_view_standardtext"><xsl:value-of select="lang_view_standardtext"/></xsl:variable>
	...
	<xsl:template match="edit">
		...
		<table cellpadding="2" cellspacing="2" width="80%" align="center">
			...
			<tr>
				...
				<td>
					<input type="text" size="40" name="values[num]" value="{value_num}" onMouseout="window.status='';return true;">
						<xsl:attribute name="onMouseover">
							<xsl:text>window.status='</xsl:text>
								<xsl:value-of select="lang_num_standardtext"/>
							<xsl:text>'; return true;</xsl:text>
						</xsl:attribute>
					</input>
				</td>
