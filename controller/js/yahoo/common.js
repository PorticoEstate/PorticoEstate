...
YAHOO.controller.serializeForm = function(formID) {
...
YAHOO.controller.formatLink = function(elCell, oRecord, oColumn, oData) { 
	var name = oRecord.getData(oColumn.key);
	var link = oRecord.getData('link');
	elCell.innerHTML = '<a href="' + link + '">' + name + '</a>'; 
};

YAHOO.controller.formatGenericLink = function() {
	links = [];
...
YAHOO.controller.inlineTableHelper = function(container, url, colDefs, options) {
	options = options || {};
	var myDataSource = new YAHOO.util.DataSource(url);
	myDataSource.responseType = YAHOO.util.DataSource.TYPE_JSON;
	myDataSource.connXhrMode = "queueRequests";
...
YAHOO.controller.radioTableHelper = function(container, url, name, selection) {
	return YAHOO.controller.checkboxTableHelper(container, url, name, selection, 'radio')
}

YAHOO.controller.checkboxTableHelper = function(container, url, name, selection, type) {
	type = type || 'checkbox';
	selection = selection || [];
	var myDataSource = new YAHOO.util.DataSource(url);
	myDataSource.responseType = YAHOO.util.DataSource.TYPE_JSON;
...
YAHOO.controller.setupDatePickers = function() {
	YAHOO.util.Dom.getElementsByClassName('date-picker', null, null, YAHOO.controller.setupDatePickerHelper, [true, false]);
	YAHOO.util.Dom.getElementsByClassName('time-picker', null, null, YAHOO.controller.setupDatePickerHelper, [false, true]);
	YAHOO.util.Dom.getElementsByClassName('datetime-picker', null, null, YAHOO.controller.setupDatePickerHelper, [true, true]);
}

YAHOO.controller.setupDatePickerHelper = function(field, args) {
	if(field._converted)
		return;
...
// Executed on all controller.uicommon-based pages
YAHOO.util.Event.addListener(window, "load", function() {
	YAHOO.controller.setupDatePickers();
});
