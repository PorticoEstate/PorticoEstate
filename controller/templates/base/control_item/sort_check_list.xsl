...
<xsl:template name="sort_check_list" xmlns:php="http://php.net/xsl">
	...
	<form action="#" id="frmSaveOrder">
						...
						<li class="drag_group list_item">
							...
							<h3>
								<a href="#">
									...
									<input type="hidden" name="group_id" value="{$control_group_id}" />
								</a>
							</h3>
							<ul class="items">
								<xsl:for-each select="control_items">
									...
									<li class="drag_item list_item">
										<a href="#">
											<span class="item_order_nr">
												<xsl:number/>
											</span>. <xsl:value-of select="title"/>
											<input type="hidden" name="item_id" value="{$control_item_id}" />
										</a>
									</li>
								</xsl:for-each>
							</ul>
		...
		<span id="saveOrder" class="pure-button pure-button-primary">
			<span class="text">Lagre rekkefølge</span>
		</span>
	</form>
</xsl:template>
