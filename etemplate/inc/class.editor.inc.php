	...
	class editor
	{
		...
		function edit($msg = '')
		{
			...
			if ($app && $app != 'etemplate' && is_array($this->extensions) &&
				(!isset($this->extensions['**loaded**']) || !is_array($this->extensions['**loaded**']) || (!isset($this->extensions['**loaded**'][$app]) || !$this->extensions['**loaded**'][$app])))
			{
				$extensions = $this->scan_for_extensions($app);
				if (count($extensions))
				{
					$msg .= (!isset($msg_ext_loaded) || !$msg_ext_loaded?lang('Extensions loaded:').' ':', ') . implode(', ',$extensions);
			...
			while (list($row,$cols) = each($this->etemplate->data))
			{
				...
				$content["Row$row"] = array(
					'height' => array("h$row" => isset($this->etemplate->data[0]["h$row"]) ? $this->etemplate->data[0]["h$row"] : ''),
					'class'  => array("c$row" => isset($this->etemplate->data[0]["c$row"]) ? $this->etemplate->data[0]["c$row"] : '')
				);
				for ($spanned = $c = 0; $c < $this->etemplate->cols; ++$c)
				{
					...
					if ($row == 1)
					{
						$content["Col$col"] = array('width' => array($col => isset($this->etemplate->data[0][$col]) ? $this->etemplate->data[0][$col] : ''));
		...
		function process_edit($content)
		{
			...
			elseif (isset($content['langfile']) && $content['langfile'])
			{
				list($name) = explode('.',$content['name']);
				if (empty($name) || !@is_dir(PHPGW_SERVER_ROOT.'/'.$name))
				{
			...
			elseif (isset($content['export_xml']) && $content['export_xml'])
			{
				$msg = $this->export_xml();
			}
			elseif (isset($content['import_xml']) && $content['import_xml'])
			{
				$msg = $this->import_xml($content['file']['tmp_name']);
			}
			elseif (isset($content['db_tools']) && $content['db_tools'])
			{
		...
		function import_xml($file)
		{
			...
			if (!isset($this->etemplate->xul_io) || !is_object($this->etemplate->xul_io))
			{
				$this->etemplate->xul_io = CreateObject('etemplate.xul_io');
			}
