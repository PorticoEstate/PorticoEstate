			...
			switch ($cell['type'])
			{
				...
				case 'select-account':	// options: #rows,{accounts(default)|both|groups},{0(=lid)|1(default=name)|2(=lid+name))}
					if ($this->firsttime)
					{
						...
						$tmp_type = ( empty($type) ) ? 'accounts' : $type;
						$this->accs = $GLOBALS['phpgw']->accounts->get_list($tmp_type); // default is accounts
					}

					reset($this->accs);
					while (list(,$acc) = each($this->accs))
					{
						if ($acc['account_type'] == 'g' || $acc['account_type'] == 'u')
				...
				case 'select-app':	// type2: ''=users enabled apps, 'installed', 'all' = not installed ones too
					...
					{
						if (!$type2 || (isset($GLOBALS['phpgw_info']['user']['apps'][$app]) && $GLOBALS['phpgw_info']['user']['apps'][$app]))
						{
