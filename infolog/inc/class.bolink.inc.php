 ...
 * at your option) any later version.					*
 \**********************************************************************/
 /* $Id: class.bolink.inc.php 17949 2007-02-13 15:02:07Z sigurdne $ */
		...
		*/
		function bolink( )
		{
			...
			if (is_array($config->config_data))
			{
				...
				if ( isset($config->config_data['send_file_ips']) )
				{
		...
		*/
		function addressbook_title( $addr )
		{
 			...
 			if (!is_array($addr))
			{
				...
				list( $addr ) = $this->contacts->get_principal_persons_data( $addr );
			}
...
// 			}
		...
		*/
		function addressbook_query( $pattern )
		{
			...
			$columns_to_display = array('contact_id', 'per_first_name', 'per_last_name', 'per_prefix', 'org_name');
			$me = $GLOBALS['phpgw_info']['user']['account_id'];

			//$addrs = $this->contacts->get_people_by_owner($me, $columns_to_display);
			$criteria = $this->contacts->criteria_for_index(
				$me, PHPGW_CONTACTS_ALL, 
				PHPGW_CONTACTS_CATEGORIES_ALL, 
				$columns_to_display, $pattern, $columns_to_display);
			$addrs = $this->contacts->get_persons($columns_to_display, '','','','','',$criteria);			
			$content = array( );
			while ($addrs && list( $key,$addr ) = each( $addrs ))
			{
				$content[$addrs[$key]['contact_id']] = $this->addressbook_title( $addr );
			}
			return $content;
		}
