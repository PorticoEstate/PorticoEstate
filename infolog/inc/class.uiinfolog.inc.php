 ...
 * at your option) any later version.					*
 \**********************************************************************/
 /* $Id: class.uiinfolog.inc.php,v 1.82 2007/02/13 19:51:38 sigurdne Exp $ */
	...
	class uiinfolog 	// UI - User Interface - HTML 
	{
		...
		function uiinfolog( )
		{
			...
			$this->icons = array(
				'type' => array(
					...
					'cancel'	=> 'cancel.png',	'cancel_alt'	=> 'Cancel',
					'confirm'	=> 'confirm.gif',	'confirm_alt'	=> 'Confirmation',
					'finish'	=> 'finish.png',	'finish_alt'	=> 'Finished',
				...
				'action' => array(
					...
					'confirm'	=> 'confirm.gif',	'confirm_alt'	=> 'Confirm',
		...
		function get_info($info,&$readonlys,$action='',$action_id='')
		{
			...
			//echo "uiinfolog::get_info: info="; _debug_array($info);
			...
			if(intval($info['info_responsible']) > 0 && $info['info_owner'] != $info['info_responsible'] && $info['info_owner'] != $GLOBALS['phpgw_info']['user']['account_id'])
			{
				$readonlys["confirm[$id]"] = !$this->bo->check_access($id,'confirm');
				$readonlys["finish[$id]"] = !$this->bo->check_access($id,'finish');
				$readonlys["cancel[$id]"] = !$this->bo->check_access($id,'cancel');
			}
			else
			{
				$readonlys["confirm[$id]"] = $readonlys["finish[$id]"]  = $readonlys["cancel[$id]"] = True;
			}
			...
			$show_links = isset($GLOBALS['phpgw_info']['user']['preferences']['infolog']['show_links']) ? $GLOBALS['phpgw_info']['user']['preferences']['infolog']['show_links'] : '';
		...
		function get_rows($query,&$rows,&$readonlys)
		{
			...
			$query['order'] = isset($query['order']) ? $query['order'] : '';
			$query['sort'] = isset($query['sort']) ? $query['sort'] : '';
			$query['ordermethod'] = isset($query['ordermethod']) ? $query['ordermethod'] : '';
		...
		function index($values = 0,$action='',$action_id='',$referer=0,$extra_app_header=False,$return_html=False)
		{
			...
			if ($action == '')
			{
				$action = isset($values['action']) && $values['action'] ? $values['action'] : get_var('action',array('POST','GET'));
				$action_id = isset($values['action_id']) && $values['action_id'] ? $values['action_id'] : get_var('action_id',array('POST','GET'));
			}
			...
			if ((isset($values['add']) && $values['add']) || (isset($values['cancel']) && $values['cancel']) || isset($values['nm']['rows']) || isset($values['main']))
			{
				if ($values['add'])
				{
				...
				else
				{
					...
					if($this->bo->emailnotification && ($do == 'confirm' || $do == 'finish' || $do == 'cancel'))
					{
		...
		*/
		function edit($content = 0,$action = '',$action_id=0,$type='',$referer='')
		{
			if (is_array($content))
			{
				...
				if ((isset($content['save']) && $content['save']) || (isset($content['delete']) && $content['delete']) || (isset($content['cancel']) && $content['cancel']))
				{
					if ((isset($content['save']) &&$content['save']) && (!$info_id || $this->bo->check_access($info_id,PHPGW_ACL_EDIT)))
					{
						$content['old_responsible'] = $this->bo->so->data['info_responsible'];
						...
						{
							if(intval($content['info_responsible']) > 0 && $content['info_responsible'] != $content['info_owner'])
							{
								if(!$info_id)
								{
					...
					elseif (isset($content['delete']) && $content['delete'] && $info_id > 0)
					{
			...
			$this->tmpl->exec('infolog.uiinfolog.edit',$content,array(
					...
					'info_confirm_status'	=> $this->bo->enums['confirm_status'],
					'info_status'			=> $this->bo->status[$content['info_type']]),
				$readonlys,array(
					'info_id'				=> $info_id,
					'info_id_parent'		=> isset($content['info_id_parent']) ? $content['info_id_parent'] : '',
					'info_link_id'			=> $content['info_link_id'],
					'action'				=> $action,
					'action_id'				=> $action_id,
					'referer'				=> $referer,
		...
		function admin()
		{
			...
			if(get_var('save',Array('POST')))
			{
				...
				$this->bo->config->config_data = array(
					...
					'emailnotification'	=> ($_POST['emailnotification']?'yes':'no')
				);
			...
			$GLOBALS['phpgw']->template->set_var(Array(
				...
				'lang_enable_emailnotification'	=> lang('enable email notification for assigned tasks'),
				'notify_checked'	=> ($this->bo->emailnotification?'checked="checked"':'')
			));
		...
		function hook_view($args)
		{
			switch ($args['location'])
			{
				case 'addressbook_view':
					$app     = 'addressbook';
					$view_id = 'ab_id';
					$view    = 'addressbook.uiaddressbook.view_person';
					break;
				case 'projects_view':
					$app     = 'projects';
					$view_id = 'project_id';
