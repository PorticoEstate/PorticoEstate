			...
			<div id="details">
				...
				<form id='activity_form' action="{$action_url}" method="post">
					...
					<dl class="proplist-col">
								...
								<dt>
									<div style="margin-bottom: 1em;">
										<label style="display:block;">
											<xsl:value-of select="php:function('lang', 'Choose another main activity for this sub activity')" />
										</label>
						...
						<xsl:choose>
							<xsl:when test="projects != ''">
								<dt>		
									<div style="margin-bottom: 1em;">
										<label style="display:block;">
											<xsl:value-of select="php:function('lang', 'Choose another project for the activity')" />
										</label>
										<select id="select_project" name="select_project">
											...
											<option value=''>
												<xsl:value-of select="php:function('lang', 'Choose another project')" />
											</option>
											<xsl:for-each select="projects">
												<option value="{id}">
													<xsl:if test="project/id = project_id">
						...
						<dt>
							<label for="name">
								<xsl:value-of select="php:function('lang','Activity name')" />
							</label>
						</dt>
						<dd>
							<xsl:choose>
						...
						<dt>
							<label for="end_date">
								<xsl:value-of select="php:function('lang', 'Responsible person')" />
							</label>
						</dt>
						<dd>
							<xsl:choose>
									...
									<select name="responsible_user_id">
										...
										<xsl:for-each select="responsible_users">
											<xsl:variable name="full_name">
												<xsl:value-of disable-output-escaping="yes" select="account_firstname"/>
												<xsl:text> </xsl:text>
