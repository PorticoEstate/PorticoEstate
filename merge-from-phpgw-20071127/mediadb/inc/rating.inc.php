<?php
  /**************************************************************************\
  * phpGroupWare - MediaDB Rating Functions                                  *
...
function add_rating_entry($order, $sort, $filter, $start, $query, $qfield)
{
    ...
    printf("        <input type=\"text\" name=\"rating_desc\""
...
function modify_rating_entry($con, $act, $order, $sort, $filter, $start, $query, $qfield)
{
    global $phpgw, $phpgw_info;

    $phpgw->db->query("select * from phpgw_mediadb_rating where rating_id=$con");
    $phpgw->db->next_record();

    $fdesc  = $phpgw->db->f("rating_desc");
    ...
    printf("<form method=POST action=\"%s\">\n",
           $phpgw->link("/mediadb/rating.php",
                        ...
                        ."&query=".urlencode($query)
			."&qfield=$qfield"));
    printf("<input type=\"hidden\" name=\"rating_id\" value=\"%s\">\n",$con);
    printf("  <table border=\"0\" cellpadding=\"0\""
           . " cellspacing=\"0\" width=75%% align=center>\n");
    printf("    <tr bgcolor=%s>\n",$color);
    printf("      <td align=left width=15%%>\n");
