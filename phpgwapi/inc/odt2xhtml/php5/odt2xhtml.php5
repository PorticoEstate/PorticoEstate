	...
	class odt2xhtml
	{
		...
		public function __construct( $root, $frontend, $file )
		{
			...
			if (ODT2XHTML_PHPCLI == TRUE)
			{
				...
				$this->dir['odf_out'] = $root . $frontend; // dir root
			...
			if (defined('IMG_SRC'))
				$this->dir['img_src']	 = IMG_SRC . 'img';
			else
				$this->dir['img_src']	 = 'img';
...
//			$this->pattern['meta'] = '`<meta (.*?) />`es';
			$this->pattern['meta'] = "/<meta (.*?) \/>/s";
			if (ODT2XHTML_FILE_CSS == FALSE)
			{
//				$this->pattern['css'] = '`<style type="text/css">(.*?)</style>`es';
				$this->pattern['css'] = "/<style type=\"text/css\">(.*?)<\/style>/s";
			}
//			$this->pattern['body']	 = '`<body>(.*?)</body>`es';
//			$this->pattern['title']	 = '`<h1 class="(.*?)">(.*?)</h1>`es';
			$this->pattern['body']	 = "/<body>(.*?)<\/body>/s";
			$this->pattern['title']	 = "<h1 class=\"(.*?)\">(.*?)<\/h1>/s";

	//		_debug_array($this->subject);
		...
		public function display_elements_html( $info, $x )
		{
			$this->code = '';
			if ($x != '0' && $x != '1')
				die($this->message->display('ko', 'bad_digit'));
			...
			if (!empty($this->elements[$info]) && is_array($this->elements[$info]))
			{
				...
				die($this->message->display('ko', 'searched_element', $info));
		}
		/*		 * *
		  privates functions
		 * * */
		...
		private function _analyze_attribute( $_attribute )
		{

			if (is_array($_attribute))
			{
				$attribute = array_pop($_attribute);
			}
			else
			{
		...
		function _choose_valid_xsl()
		{
			...
			if (ODT2XHTML_DEBUG == TRUE)
			{
				echo $this->message->display('ok', 'valid_xsl');
			}
		}
		/*		 * * create file css ** */
		...
		function _create_file_css()
		{
			...
			if (ODT2XHTML_DEBUG == TRUE)
			{
				echo $this->message->display('ok', 'creating_file_css');
			}
		}
		...
		function _create_files()
		{
			...
			if (ODT2XHTML_DEBUG == TRUE)
			{
				echo $this->message->display('ok', 'creating_file_html');
			}
		}
		...
		private function _make_new_xml_with_odf_files()
		{
			...
			if (file_exists($this->file['xml']) && is_readable($this->file['xml']))
			{
				...
				if (ODT2XHTML_DEBUG == TRUE)
				{
					echo $this->message->display('ok', 'making_meta');
				}
			}
			else
				die($this->message->display('ko', 'pb_file', $this->file['xml']));
			...
			if (file_exists($this->file['xml']) && is_readable($this->file['xml']))
			{
				...
				if (ODT2XHTML_DEBUG == TRUE)
				{
					echo $this->message->display('ok', 'making_style');
				}
			}
			else
				die($this->message->display('ko', 'pb_file', $this->file['xml']));
			...
			if (file_exists($this->file['xml']) && is_readable($this->file['xml']))
			{
				...
				switch ($this->file['ext'])
				{
					...
					case 'ott' :
						...
						{
							$this->content	 = $this->_replace_content('header');
						}
						# modify src img
						...
						{
							$this->content	 = str_replace('</office:text>', $this->footer . '</office:text>', $this->content);
						}
						break;
					case 'sxw' :
					case 'stw' :
						...
						# add footer page
						if (!empty($this->footer))
							$this->content	 = str_replace('</office:body>', $this->footer . '</office:body>', $this->content);
						break;
				}
				...
				if (ODT2XHTML_DEBUG == TRUE)
				{
					echo $this->message->display('ok', 'making_content');
				}
			}
			else
			{
			...
			if (ODT2XHTML_DEBUG == TRUE)
			{
				echo $this->message->display('ok', 'file_xml');
			}
		}
		...
		private function _mk_all_dir()
		{
			...
			{
				if (!mkdir($this->dir['odf_tmp'], 0777, true))
				{
					die($this->message->display('ko', 'making_dir_tmp', $this->dir['odf_tmp']));
				}
				else
				{
					if (ODT2XHTML_DEBUG == TRUE)
					{
						echo $this->message->display('ok', 'making_dir_tmp', $this->dir['odf_tmp']);
					}
				}
			}
			...
			{
				if (!mkdir($this->dir['img'], 0777, true))
				{
					die($this->message->display('ko', 'making_dir_img', $this->dir['img']));
				}
				else
				{
					if (ODT2XHTML_DEBUG == TRUE)
					{
						echo $this->message->display('ok', 'making_dir_img', $this->dir['img']);
					}
				}
			}
			...
			{
				if (!mkdir($this->dir['img_php'], 0777, true))
				{
					die($this->message->display('ko', 'making_dir_img_php', $this->dir['img_php']));
				}
				else
				{
					if (ODT2XHTML_DEBUG == TRUE)
					{
						echo $this->message->display('ok', 'making_dir_img_php', $this->dir['img_php']);
					}
				}
			}
			...
			{
				if (!mkdir($this->dir['img_php_ext'], 0777, true))
					die($this->message->display('ko', 'making_dir_imp_php_ext', $this->dir['img_php_ext']));
				else
				{
					if (ODT2XHTML_DEBUG == TRUE)
					{
						echo $this->message->display('ok', 'making_dir_img_php_ext', $this->dir['img_php_ext']);
					}
				}
			}
			...
			{
				if (!mkdir($this->dir['img_OOo'], 0777, true))
				{
					die($this->message->display('ko', 'making_dir_img_ooo', $this->dir['img_OOo']));
				}
				else
				{
					if (ODT2XHTML_DEBUG == TRUE)
					{
						echo $this->message->display('ok', 'making_dir_img_ooo', $this->dir['img_OOo']);
					}
				}
			}
			...
			{
				if (!mkdir($this->dir['html'], 0777, true))
				{
					die($this->message->display('ko', 'making_dir_html', $this->dir['html']));
				}
				else
				{
					if (ODT2XHTML_DEBUG == TRUE)
					{
						echo $this->message->display('ok', 'making_dir_html', $this->dir['html']);
					}
				}
			}
			...
			if (ODT2XHTML_DEBUG == TRUE)
			{
				echo $this->message->display('ok', 'making_all_dir');
			}
		}
		/*		 * * modify appel css ** */
		...
		private function _modify_title()
		{
			...
			{
				$this->title .= $this->file['name'] . '.' . $this->file['ext'];
			}
			$this->title .= '&quot;';
		...
		private function _mv_icon()
		{
			...
			{
				if (!copy(ODT2XHTML_ROOT . '/' . $v, $this->dir['img'] . '/' . $v))
				{
					echo $this->message->display('ko', 'icon_copy', $v);
				}
				else if (ODT2XHTML_DEBUG == TRUE)
				{
				...
				while (false !== $this->file['img'] = readdir($this->handle))
				{
					if (is_file($this->dir['pictures'] . '/' . $this->file['img']))
					{
						...
						if (rename($this->dir['pictures'] . '/' . $this->file['img'], $this->dir['img_OOo'] . '/' . $this->file['img']))
						{
							...
							if (ODT2XHTML_DEBUG == TRUE)
							{
								echo $this->message->display('ok', 'moving_img', $this->file['img']);
							}
						}
						else
						{
							die($this->message->display('ko', 'moving_img', $this->file['img']));
						}
					}
				}
		...
		private function _unzip_odf()
		{
			...
			if (ODT2XHTML_DEBUG == TRUE)
			{
				echo $this->message->display('ok', 'unzip');
			}
		}
		...
		private function _valid_ext()
		{
			...
			if (ODT2XHTML_DEBUG == TRUE)
			{
				echo $this->message->display('ok', 'extension');
			}
		}
		/*		 * *
		  Protected functions
