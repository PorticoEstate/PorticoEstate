...
function initLM(config) {
	...
	try
	{
		...
		{
			widthLeft=config.unitLeftWidth;
		}

		if( typeof config.unitRightWidth != 'undefined')
		{
			widthRight=config.unitRightWidth;
		}
		...
		var layout = new YAHOO.widget.Layout({
			...
			minHeight: 400,
            units: [
				{ position: 'top', body: north, height: 32 },
				{ position: 'left', header: getHeader(west), body: west, width: widthLeft, resize: true, scroll: true, gutter: "0px 5px 0px 0px", collapse: false, maxWidth: 300, minWidth: 6 },
                { position: 'center', header: getHeader(center), body: center, scroll: true },
                { position: 'right', header: getHeader(east), body: east, width: widthRight, resize: true, scroll: true, gutter: "0px 0px 0px 5px", collapse: false, maxWidth: 300, minWidth: 6 },
                { position: 'bottom', body: south, height: 32 }
            ]
        });
        layout.render();
