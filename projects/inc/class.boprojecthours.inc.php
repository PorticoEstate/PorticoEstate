<?php
	...
	* @package projects
	* @version $Id: class.boprojecthours.inc.php,v 1.43 2006/12/05 19:40:45 sigurdne Exp $
	...
	*/
	...
	class boprojecthours
	{
		...
		{
			$filter = $this->filter;
			$hours_list = $this->sohours->read_hours(array('start' => $this->start,
			                                               'limit' => $this->limit,
			                                               'query' => $this->query,
			                                               'filter' => $filter,
			                                               'sort' => $this->sort,
		...
		function check_values($values)
		{
			...
			{
				if( (intval($values['hours']) == 0 && intval($values['minutes']) == 0) &&
			      (intval($values['t_journey_h']) == 0 && intval($values['t_journey_m']) == 0)
			    )
				{
		...
		function check_ttracker($values)
		{
			...
			{
				$is_active = $this->sohours->check_ttracker(array('project_id' => $values['project_id'],'status' => 'active'));
				if($is_active)
				{
		...
		function ttracker($values)
		{
			...
			{
				case 'save':	$this->sohours->save_ttracker(); break;
				default:		$this->sohours->ttracker($values);break;
			}
		}
		...
		function build_import_controlling_sheet($lines, &$error)
		{
			for($i=0; $i < count($lines); $i++)
			{
				...
				if($i > 3) //here we go
				{
					...
					$project = explode("\t", $lines[$i]);	++$i;
					$journey = explode("\t", $lines[$i]); ++$i;
					...
					for($j=5; $j < count($project); $j++)
					{
						...
						if(($project[$j] > 0) || ($journey[$j] > 0))
						{
							...
							$savematrix[] = array('employee'     => $accountinfo[2],
							                      'projectid'    => $project[0],
