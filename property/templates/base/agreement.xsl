				...
				<form ENCTYPE="multipart/form-data" class="pure-form pure-form-aligned" id="form" method="post" name="form" action="{$edit_url}">
					<fieldset>
						...
						<div class="pure-control-group">
							...
							<input type="text" name="values[contract_id]" value="{value_contract_id}" class="pure-input-3-4">
								<xsl:attribute name="title">
									<xsl:value-of select="php:function('lang', 'contract')"/>
								</xsl:attribute>
							</input>
						...
						<div class="pure-control-group">
							...
							<select name="values[group_id]" title="{$lang_agreement_group_statustext}" class="pure-input-3-4">
								<xsl:attribute name="data-validation">
									<xsl:text>required</xsl:text>
								</xsl:attribute>
								<option value="">
						...
						<xsl:choose>
							<xsl:when test="member_of_list != ''">
								<div class="pure-control-group">
									...
									<select name="values[member_of][]" disabled="disabled" multiple="multiple" title="{$lang_member_of_statustext}" class="pure-input-3-4">
