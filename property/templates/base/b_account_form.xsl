<!-- $Id$ -->
	...
	<xsl:template match="b_account_data">
		<script type="text/javascript">
			...
			{
				Window1=window.open('<xsl:value-of select="b_account_link"></xsl:value-of>',"Search","left=50,top=100,width=800,height=700,toolbar=no,scrollbars=yes,resizable=yes");
			}		
		</script>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<tr>
					<td valign="top">
						<a href="javascript:b_account_lookup()" onMouseover="window.status='{lang_select_b_account_help}';return true;" onMouseout="window.status='';return true;"><xsl:value-of select="lang_b_account"></xsl:value-of></a>
					</td>
					<td>
						<input size="9" type="text" name="b_account_id" value="{value_b_account_id}">
							<xsl:attribute name="title">
								<xsl:value-of select="lang_select_b_account_help"></xsl:value-of>
							</xsl:attribute>
						</input>
