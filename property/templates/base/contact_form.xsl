...
<xsl:template match="contact_data">
	<script type="text/javascript">
		...
		{
			TINY.box.show({iframe:'<xsl:value-of select="contact_link"/>', boxid:"frameless",width:Math.round($(window).width()*0.9),height:Math.round($(window).height()*0.9),fixed:false,maskid:"darkmask",maskopacity:40, mask:true, animate:true, close: true});
		}
