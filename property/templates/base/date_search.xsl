<!-- $Id$ -->
	...
	<xsl:template name="app_data">
		<xsl:apply-templates select="date_search"></xsl:apply-templates>
	</xsl:template>
			...
			<table width="100%" cellpadding="2" cellspacing="2" align="center">
				<tr>
					...
					<td>
						...
						<img id="start_date-trigger" src="{img_cal}" alt="{lang_datetitle}" title="{lang_datetitle}" style="cursor:pointer; cursor:hand;"></img>
				...
				<tr>
					...
					<td>
						...
						<img id="end_date-trigger" src="{img_cal}" alt="{lang_datetitle}" title="{lang_datetitle}" style="cursor:pointer; cursor:hand;"></img>
					</td>
				</tr>
				<tr>
					<td class="small_text" valign="top">
						...
						<input type="button" name="convert" value="{$lang_submit}" onClick="ExchangeDate(this.form);" onMouseout="window.status='';return true;">
							<xsl:attribute name="onMouseover">
								<xsl:text>window.status='</xsl:text>
								<xsl:value-of select="lang_submit_statustext"></xsl:value-of>
								<xsl:text>'; return true;</xsl:text>
							</xsl:attribute>
						</input>
					</td>
