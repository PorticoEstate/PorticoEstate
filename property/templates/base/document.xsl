<!-- $Id$ -->
	...
	<xsl:template name="doc_type_filter">
		...
		<form method="post" action="{select_action}">
			<select name="doc_type" onChange="this.form.submit();" onMouseout="window.status='';return true;">
				...
				<option value=""><xsl:value-of select="lang_no_doc_type"/></option>
	...
	<xsl:template match="table_header_document">
		<xsl:variable name="sort_document_name"><xsl:value-of select="sort_document_name"/></xsl:variable>
		<tr class="th">
			<td class="th_text" width="10%" align="left">
				<a href="{$sort_document_name}"><xsl:value-of select="lang_document_name"/></a>
			</td>
	...
	<xsl:template match="values_document">
		<xsl:variable name="lang_view_file_statustext"><xsl:value-of select="lang_view_file_statustext"/></xsl:variable>
		...
		<xsl:variable name="lang_history_statustext"><xsl:value-of select="//lang_history_statustext"/></xsl:variable>
			...
			<td align="left">
				...
				<xsl:choose>
					<xsl:when test="link_to_files!=''">
						...
						<a href="{$link_to_file}" target="_blank" onMouseover="window.status='{lang_view_file_statustext}';return true;" onMouseout="window.status='';return true;"><xsl:value-of select="document_name"/></a>
					</xsl:when>
					<xsl:otherwise>
						<xsl:variable name="link_view_file"><xsl:value-of select="link_view_file"/></xsl:variable>
						<a href="{$link_view_file}" target="_blank" onMouseover="window.status='{$lang_view_file_statustext}';return true;" onMouseout="window.status='';return true;"><xsl:value-of select="document_name"/></a>
					</xsl:otherwise>
				</xsl:choose>
			</td>
			<td align="left">
				...
				<form ENCTYPE="multipart/form-data" method="post" name="form" action="{$form_action}">
					...
					<tr>
						...
						<td>
							...
							<img id="values_document_date-trigger" src="{img_cal}" alt="{lang_datetitle}" title="{lang_datetitle}" style="cursor:pointer; cursor:hand;"/>
						</td>
					</tr>
					<tr>
						<td>
			...
			<table width="80%" cellpadding="2" cellspacing="2" align="center">
				<xsl:choose>
					...
					<xsl:otherwise>
						...
						<tr><td class="th_text" colspan="3"><div id="paging_0"/><div id="datatable-container_0"/></td></tr>	
