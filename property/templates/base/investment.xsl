<!-- $Id$ -->
	...
	<xsl:template name="app_data">
		<xsl:choose>
			...
			<xsl:when test="history">
				<xsl:apply-templates select="history"></xsl:apply-templates>
			</xsl:when>
			<xsl:otherwise>
					...
					<xsl:when test="values[entity_id]!=''">
						...
						<tr>
							...
							<td class="th_text" align="right">
								<xsl:value-of select="sum_initial_value"></xsl:value-of>
							</td>
							<td class="th_text" align="right">
								<xsl:value-of select="sum_value"></xsl:value-of>
							</td>
							<td></td>
							<td></td>
							<td></td>
	...
	<xsl:template match="table_header">
		<tr class="th">
			...
			<td class="th_text" width="10%" align="right">
				<xsl:value-of select="lang_entity_id"></xsl:value-of>
			</td>
			<td class="th_text" width="5%" align="right">
				<xsl:value-of select="lang_investment_id"></xsl:value-of>
			</td>
			<td class="th_text" width="10%">
				<xsl:value-of select="lang_descr"></xsl:value-of>
			</td>
			<td class="th_text" width="10%">
				<xsl:value-of select="lang_entity_name"></xsl:value-of>
			</td>
			<td class="th_text" width="20%" align="center">
				<xsl:value-of select="lang_initial_value"></xsl:value-of>
			</td>
			<td class="th_text" width="20%" align="center">
			...
			<td class="th_text" width="10%" align="center">
				<xsl:value-of select="lang_write_off"></xsl:value-of>
			</td>
			<td class="th_text" width="10%" align="center">
	...
	<xsl:template match="values">
		...
		<tr>
			...
			<td align="right">
				<input type="hidden" name="values[entity_id][{counter}]" value="{entity_id}"></input>
				<input type="hidden" name="values[investment_id][{counter}]" value="{investment_id}"></input>
				<input type="hidden" name="values[initial_value][{counter}]" value="{initial_value_ex}"></input>
				<input type="hidden" name="values[value][{counter}]" value="{value_ex}"></input>
				<xsl:value-of select="district_id"></xsl:value-of>
			</td>

			<td align="left">
				<xsl:value-of select="part_of_town"></xsl:value-of>
			</td>

			<td align="right">
				<xsl:value-of select="entity_id"></xsl:value-of>
			</td>
			<td align="right">
				<xsl:value-of select="investment_id"></xsl:value-of>
			</td>
			<td align="left">
			...
			<td align="right">
				<xsl:value-of select="this_write_off"></xsl:value-of>
			</td>
			<td align="right">
			...
			<xsl:choose>
				<xsl:when test="is_admin=1">
					<td align="center">
						...
						<a href="{$link_history}" onMouseover="window.status='{$lang_history_statustext}';return true;" onMouseout="window.status='';return true;"><xsl:value-of select="lang_history"></xsl:value-of></a>
					</td>
					<td>
						<xsl:choose>
...
<!-- History -->
	...
	<xsl:template match="history">
				...
				<xsl:when test="entity_type!=''">
					<tr>
						<td width="25%" class="th_text" align="left">
							<xsl:value-of select="lang_entity_type"></xsl:value-of>
						</td>
						<td width="25%" class="th_text" align="left">
		...
		<form method="post" name="form" action="{$update_action}">
			<table width="100%" cellpadding="2" cellspacing="2" align="center">
				<input type="hidden" name="values[update][0]" value="0"></input>
				<input type="hidden" name="values[entity_id][0]" value="{entity_id}"></input>
				<input type="hidden" name="values[investment_id][0]" value="{investment_id}"></input>
				<xsl:apply-templates select="table_header_history"></xsl:apply-templates>
				<xsl:apply-templates select="values_history"></xsl:apply-templates>
			...
			<xsl:choose>
				<xsl:when test="is_admin=16 and current_index=1">
					<td align="center">
						...
						<a href="{$link_delete}" onMouseover="window.status='{$lang_delete_statustext}';return true;" onMouseout="window.status='';return true;"><xsl:value-of select="lang_delete"></xsl:value-of></a>
						<input type="hidden" name="values[initial_value][0]" value="{initial_value_ex}"></input>
						<input type="hidden" name="values[value][0]" value="{value_ex}"></input>
					</td>
				</xsl:when>
				<xsl:otherwise>
					<td>
...
<!-- add -->
	...
	<xsl:template match="add">
		<script type="text/javascript">
			...
			{
			Window1=window.open('<xsl:value-of select="location_link"></xsl:value-of>',"Search","left=50,top=100,width=800,height=700,toolbar=no,scrollbars=yes,resizable=yes");
			}		
		</script>
		<div align="left">		
			<table cellpadding="2" cellspacing="2" width="80%" align="center">
				...
				<form method="post" action="{$form_action}" name="form">
					...
					<tr>
						<td>
							<xsl:value-of select="lang_write_off_period"></xsl:value-of>
						</td>
						<td>
							<xsl:call-template name="cat_select"></xsl:call-template>
							<xsl:text>  </xsl:text>
							<xsl:value-of select="lang_new"></xsl:value-of>

							<input type="text" name="values[new_period]" value="{value_new_period}" size="3" onMouseout="window.status='';return true;">
								<xsl:attribute name="onMouseover">
									<xsl:text>window.status='</xsl:text>
									<xsl:value-of select="lang_new_period_statustext"></xsl:value-of>
									<xsl:text>'; return true;</xsl:text>
								</xsl:attribute>
							</input>
					...
					<tr>
						...
						<td>
							<input type="text" name="values[initial_value]" value="{value_inital_value}" onMouseout="window.status='';return true;">
								<xsl:attribute name="onMouseover">
									<xsl:text>window.status='</xsl:text>
									<xsl:value-of select="lang_value_statustext"></xsl:value-of>
									<xsl:text>'; return true;</xsl:text>
								</xsl:attribute>
							</input>
