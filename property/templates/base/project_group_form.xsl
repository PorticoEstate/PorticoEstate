...
<xsl:template xmlns:php="http://php.net/xsl" match="project_group_data">
		<script type="text/javascript">
			...
			{
		TINY.box.show({iframe:'<xsl:value-of select="project_group_url"/>', boxid:"frameless",width:Math.round($(window).width()*0.9),height:Math.round($(window).height()*0.9),fixed:false,maskid:"darkmask",maskopacity:40, mask:true, animate:true, close: true});
			}
		</script>
