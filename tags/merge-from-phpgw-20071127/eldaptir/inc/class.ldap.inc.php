	...
	class ldap
	{
		...
		function ldap($serverinfo='')
		{
			if(gettype($serverinfo) == 'array')
			{
				...
				if($this->DEBUG) { echo '<br>Type: '.$this->type; }
				$this->schema = CreateObject('eldaptir.schema',$this->type);
				$this->schema->ldap = $this->ldap;
			}
			else
			{
				...
				$this->schema = CreateObject('eldaptir.schema');
				$this->schema->ldap = $this->ldap;
			}
		...
		*/
		...
		function exists($entry)
		{
			...
			$sr = @ldap_search($GLOBASL['ldap'],$this->base,$filter,array("uid"));
