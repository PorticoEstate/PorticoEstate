...
* @package etemplate
* @version $Id: class.xul_io.inc.php 17950 2007-02-13 15:30:58Z sigurdne $
*/
	...
	class xul_io
	{
		...
		function import(&$etempl,$data)
		{
			...
			while (list($n,$node) = each($vals))
			{
				$type = $node['type'];
				$tag = $node['tag'];
				$attr = isset($node['attributes']) && is_array($node['attributes']) ? $node['attributes'] : array();
				$attr['span'] = isset($attr['span']) ? $attr['span'] : ''; //Fix this
				...
				if ($tag == 'grid' && $type == 'complete' && (!isset($tab_attr) || !is_array($tab_attr)))
				{
					$tag = 'template';
				}
				if ($tag != 'textbox' && !isset($attr['type']))
				{
					$attr['type'] = isset($this->xul2widget[$tag]) && $this->xul2widget[$tag] ? $this->xul2widget[$tag] : $tag;
				}
				if ($this->debug)
				{
				...
				switch ($tag)
				{
					case 'overlay':
						break;
					case 'grid':
						if ($type != 'close' && isset($tab_attr) && is_array($tab_attr))
						{
							$tab_names[] = $attr['name'];
							break;
						}
						...
						if (isset($grid_started) && $grid_started)	// more than one grid in the file --> place it into the cache
						{
						...
						{
							$size = (isset($attr[$opt]) ? $attr[$opt] : '') . ($size != '' ? ",$size" : '');
						}
						$etempl->size = $size;
					...
					case 'column':
						...
						$etempl->data[0][$etempl->num2chrs($etempl->cols++)] = (isset($attr['width']) ? $attr['width'] : '') .
					...
					case 'row':
						...
						$r = ++$etempl->rows;
						$col = 0;
						$etempl->data[0]["c$r"] = (isset($attr['class']) ? $attr['class'] : '') . (isset($attr['valign']) && $attr['valign'] ? ','.$attr['valign'] : '');
						$etempl->data[0]["h$r"] = isset($attr['height']) ? $attr['height'] : '' .
							(isset($attr['disabled']) && $attr['disabled'] ? ','.$attr['disabled'] : '');
						break;
					case 'styles':
					...
					case 'menupopup':
						...
						{
							$attr['help'] = isset($attr['statustext']) ? $attr['statustext'] : '';
					...
					case 'hbox':
						...
						{
							$cell = &$box[$node['level']];
							$cell['size'] = $cell['anz'] . (isset($cell['size']) && $cell['size'] != '' ? ','.$cell['size'] : '');
							unset($cell['anz']);
					...
					default:
						switch ($tag)
						{
							case 'description':
							case 'label':
								$attr['label'] = isset($attr['value']) ? $attr['value'] : '';
								unset($attr['value']);
								break;
							case 'template':
							...
							case 'listbox':
								...
								break;
							case 'button':
								if ((isset($attr['image']) && $attr['image']) || (isset($attr['ro_image']) && $attr['ro_image']))
								{
									$attr['size'] = $attr['image'] . (isset($attr['ro_image']) && $attr['ro_image'] ? ','.$attr['ro_image'] : '');
									unset($attr['image']); unset($attr['ro_image']);
								}
								break;
						}
						$attr['help'] = isset($attr['statustext']) ? $attr['statustext'] : ''; unset($attr['statustext']);
						$attr['span'] .= isset($attr['class']) && $attr['class'] ? ','.$attr['class'] : ''; unset($attr['class']);
						if ($type == 'close')
						{
							break;
						}
