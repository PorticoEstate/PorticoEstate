<?php
	...
	* @version $Id: class.wsdl.inc.php 17829 2006-12-28 18:03:40Z Caeies $
	*/
...
class phpgwapi_wsdl extends phpgwapi_nusoap_base {
     ...
     * @access public 
     */
    function phpgwapi_wsdl($wsdl = '',$proxyhost=false,$proxyport=false,$proxyusername=false,$proxypassword=false,$timeout=0,$response_timeout=30){
    ...
    function start_element($parser, $name, $attrs)
    {
        if ($this->status == 'schema') {
            ...
            $this->status = 'schema';
            $this->currentSchema = createObject ("phpgwapi.soap_XMLSchema",'', '', $this->namespaces);
            $this->currentSchema->schemaStartElement($parser, $name, $attrs);
            $this->appendDebug($this->currentSchema->getDebug());
            $this->currentSchema->clearDebug();
        } else {
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
		...
		if (strpos($type, ':')) {
			...
			if($ns == $this->XMLSchemaVersion || $ns == 'http://schemas.xmlsoap.org/soap/encoding/'){
				...
				// check type isn't a custom type extending soap_XMLSchema namespace
