...
<table width=95%>
	<form name="maintain_questions" method="POST" action="{form_maintain_questions_action}">
		<tr bgcolor="#D3DCE3">
			<th>{head_summary}</th><th>{head_details}</th><th>{head_date}</th><th>{head_author}</th><th width=80>{lang_actions}&nbsp;&nbsp;<a href="javascript:check_all('select')"><img src="{img_src_checkall}"></a></th>
		</tr>
		<!-- BEGIN table_row_block -->
		<tr bgcolor="{tr_color}">
			<td>{summary}</td><td>{details}</td><td>{date}</td><td>{author}</td><td align=right>{actions}<input type="checkbox" name="{name_checkbox}" value="True"></td>
		</tr>
		<!-- END table_row_block -->
		<tr>
