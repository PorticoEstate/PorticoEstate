...
function start_drag(drag_type){
	...
	$(document).bind("mousemove", function(e){
		...
		if( $(next_elem).length > 0 && e.pageY > $(next_elem).offset().top ){
			...
			console.log("Over next elem...  e.pageY " + e.pageY + " og next_elem.offset().top " + $(next_elem).offset().top);
		...
		// Move drag element over previous element
			...
			if( e.pageY < ($(prev_elem).offset().top + $(prev_elem).height()/2) ){
				$(placeholder).insertBefore(prev_elem);
				prev_elem = $(placeholder).prev();
				next_elem = $(placeholder).next();
				console.log("prev_elem.length: "  + $(prev_elem).length);
				console.log("prev_elem: "  + $(prev_elem).text());
				console.log("Over prev elem...  e.pageY " + e.pageY + " og next_elem.offset().top " + ($(prev_elem).offset().top + $(prev_elem).height()/2) );			
