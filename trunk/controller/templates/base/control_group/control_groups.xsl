...
<xsl:template name="control_groups" xmlns:php="http://php.net/xsl">
  ...
  <div class="yui-content tab_content">
    <div id="control_groups">
      ...
      <h2>
        <xsl:value-of select="control_area/title"/>
      </h2>
      ...
      <form id="frm_save_control_groups" action="index.php?menuaction=controller.uicontrol.save_control_groups" method="post">
        <xsl:variable name="control_area_id">
          <xsl:value-of select="control_area/id"/>
        </xsl:variable>
          ...
          <xsl:for-each select="//control_groups">
            ...
            <xsl:choose>
              <xsl:when test="checked = 1">
                <li>
                  <input type="checkbox" checked="checked" name="control_group_ids[]" value="{$control_group_id}" />
                  <xsl:value-of select="control_group/group_name"/>
                </li>
              </xsl:when>
              <xsl:otherwise>
                <li>
                  <input type="checkbox" name="control_group_ids[]" value="{$control_group_id}" />
                  <xsl:value-of select="control_group/group_name"/>
                </li>
              </xsl:otherwise>
            </xsl:choose>
