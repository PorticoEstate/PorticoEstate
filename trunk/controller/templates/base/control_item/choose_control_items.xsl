  ...
  <div class="yui-content tab_content">
    <div>
      ...
      <div class="expand_menu">
        ...
        <div class="collapse_all focus">Skjul alle</div>
      </div>
      ...
      <form id="frm_control_items" action="index.php?menuaction=controller.uicontrol.save_control_items" method="post">	
        ...
        <xsl:variable name="control_group_ids">
          <xsl:value-of select="control_group_ids"/>
        </xsl:variable>
            ...
            <li>
              <xsl:choose>
                <xsl:when test="group_control_items/child::node()">
                  <h4>
                    <img src="controller/images/arrow_right.png" width="14"/>
                    <span>
                      ...
                      <xsl:choose>
                        <xsl:when test="checked = 1">
                          <li>
                            ...
                            <input type="checkbox"  checked="checked" id="ch_{$control_group_id}:{$control_item_id}" value="{$control_group_id}:{$control_item_id}" />
                        ...
                        <xsl:otherwise>
                          <li>
                            ...
                            <input type="checkbox"  id="ch_{$control_group_id}:{$control_item_id}" value="{$control_group_id}:{$control_item_id}" />
                            <xsl:value-of select="control_item/title"/>
                          </li>
                        </xsl:otherwise>
                      </xsl:choose>
