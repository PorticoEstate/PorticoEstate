<!-- $Id: help.xsl,v 1.2 2007/03/06 13:38:48 sigurdne Exp $ -->
		...
		<table cellpadding="0" cellspacing="0" width="100%">
 			<tr class="th">
  				<td class="th_text" valign="middle">
					<img src="{$img}" border="0"/><xsl:text> </xsl:text>
					<xsl:value-of select="title"/><xsl:text> </xsl:text>
					<xsl:choose>
						<xsl:when test="version != ''">
							<xsl:value-of select="lang_version"/><xsl:text> </xsl:text><xsl:value-of select="version"/>
						</xsl:when>
					</xsl:choose>
				</td>
