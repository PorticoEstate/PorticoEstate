<?php
	...
	* @package phpgwapi
	* @subpackage gui
	* @version $Id: class.sbox.inc.php,v 1.31 2006/12/12 15:17:50 sigurdne Exp $
	*/
	...
	*/
	class sbox
	{
		...
		function getMonthText($name, $selected=0)
		{
			...
			{
				$out .= '<option value="'.$i.'"'.($selected!=$i?'':' selected').'>'.($this->monthnames[$i]!=''?lang($this->monthnames[$i]):lang('month')).'</option>'."\n";
			}
			return '<select name="'.$name.'">'."\n".$out.'</select>'."\n";
		}
		...
		function getDays($name, $selected=0)
		{
			...
			for($i=0;$i<32;$i++)
			{
				$out .= '<option value="'.($i?$i:'').'"'.($selected!=$i?'':' selected').'>'.($i?$i:lang('day')).'</option>'."\n";
			}
			return '<select name="'.$name.'">'."\n".$out.'</select>'."\n";
		}
		...
		function getYears($name, $selected = 0, $startYear = 0, $endYear = 0)
		{
			...
			if ($selected && $startYear > $selected)
			{
				$startYear = $selected;
			}
			...
			*/
			...
			$out .= '<option value="">'.lang('year').'</option>'."\n";
