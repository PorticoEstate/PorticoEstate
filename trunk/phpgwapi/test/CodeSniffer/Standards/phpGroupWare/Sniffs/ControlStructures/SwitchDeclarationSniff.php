...
/**
 * phpGroupWare_Sniffs_ControlStructures_SwitchDeclarationSniff.
 *
 ...
 * @link	  http://pear.php.net/package/PHP_CodeSniffer
 */
class phpGroupWare_Sniffs_ControlStructures_SwitchDeclarationSniff implements PHP_CodeSniffer_Sniff
{
	...
	public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
	{
		...
		$nextCase      = $stackPtr;
		$caseAlignment = ($switch['column'] + 1); // working with tabs
		$caseCount     = 0;
		...
		while (($nextCase = $phpcsFile->findNext(array(T_CASE, T_SWITCH), ($nextCase + 1), $switch['scope_closer'])) !== false)
		{
			...
			{
				$error = $tokens[$nextCase]['column'] . ': CASE keyword must be indented 4 spaces from SWITCH keyword';
				$phpcsFile->addError($error, $nextCase);
			}
