...
// BUDGET FOR ACTIVIES
		...
		function get_sub_projects($params)
		{
			switch($params['page'])
			{
				case 'planned':			$column = 'project_id,parent,level,budget,time_planned'; break;
				case 'hours':
//				case 'budget':			$column = 'project_id,accounting,acc_factor,billable,level'; break;
				case 'budget':			$column = 'project_id,parent,accounting,acc_factor,billable,level,budget,time_planned'; break;
			}
		...
		function list_projects($params)
		{
			...
			{
				$pro_list = $this->soprojects->read_gantt_projects(array('project_id' => $params['project_id'],'parent_array' => $params['parent_array']));
			}
			else
			{
...
*/
		...
		function formatted_edate($edate = 0,$colored = True,$type = 'project')
		{
			...
			if($this->html_output && $colored)
			{
				switch($type)
				{
					case 'ms':	$event = 'milestone date due'; break;
					default:	$event = 'project date due'; break;
				}
...
*/
		...
		function check_values($action, $values)
		{
			...
			// get sub jobs
			$subs = $this->get_sub_projects(array('project_id' => $values['project_id']));
...
// ------------ ALARM ----------------
		...
		function action_format($selected = 0,$action = 'role',$type = '')
		{
			...
			switch($action)
			{
				...
				case 'charge':	$list = $this->list_surcharges(); break;
		...
		*/
		function async_workhours_booking($data=array())
		{
			...
			if( !$data || !is_array($data) ||
			   !isset($data['book_type']) || !isset($data['book_month']) || !isset($data['book_year'])
			  )
				return false;
		...
		*/
		function async_worktime_statusmail($data=array())
		{
			...
			if(($data == false) || !is_array($data) ||
			   !isset($data['mail_type']) || !isset($data['last_date'])
			  )
			{
				return false;
			}
			...
			if(is_array($employees))
			{
				...
				while(list($employee_id, $employee_projects) = each($employees))
				{
					...
					if( (isset($prefs->data['projects']['send_status_mail'])) &&
		...
		function async_worktime_warnmail($data=array())
		{
			...
			if(($data == false) || !is_array($data) ||
			   !isset($data['warnmail_type']) || !isset($data['warnmail_month']) || !isset($data['warnmail_year'])
			  )
				return false;
...
//				$country = ucfirst($GLOBALS['phpgw']->translation->retranslate($sbox->country_array[$pref_country]));
				...
				$acc_data = $this->soconfig->read_employees(array('start'=>0, 'limit'=>false, 'account_id'=>$employee_id, 'date' => mktime(0,0,0,$warnmail_month,1,$warnmail_year)));
				...
				$body .= 'bitte die Stunden für den Zeitraum '.$timelen.' nachtragen.'.$newline.$newline;
				$body .= '   wöchentliche Arbeitszeit:'.$this->format_string($weekly_workhours_formated, 10, ' ', ' ', STR_PAD_LEFT).$newline;
		...
		*/
		function get_cost_accounting_diamant($month,$year,$location_id)
		{
			...
			for ($i = 0; $i < $max; ++$i)
			{
				if (ereg('^I',$list[$i]['p_number'])) // Intern
				{
					...
					{
						$cost_unit = ''; // Kostenträger (Projekt)
				...
				else if (ereg('^P',$list[$i]['p_number'])) // Projekt
				{
					...
					{
						$charge_cost_centre = '';
						$cost_unit = substr($list[$i]['p_number'],1); // Kostenträger (Projekt)
					  ...
					  if ($office != $location_data['location_ident'])
						{
							...
							$cost_unit =  '04' . $location['location_ident'] . '9' . $location['location_custnum']; // Problem mit Jahreszahl bei Geschäftsjahresübergreifenden Projekten!
						}
				...
				else if (ereg('^D',$list[$i]['p_number'])) // Dienstleistung
				{
					...
					{
						$charge_cost_centre = '';
						$cost_unit = substr($list[$i]['p_number'],1); // Kostenträger (Kunde)
						$office = substr($cost_unit,2,2); // Niederlassungsnummer
					    if ($office != $location_data['location_ident'])
						{
							$location = $this->soconfig->get_location_for_ident($office);
		...
		function get_interactions($env)
		{
			...
			*/
			...
			if(($env['status'] == 'active') &&
			   !($env['action'] == 'subs' && !((int) $env['pro_main'])) && 
			   $this->add_perms(array('action'  => $env['action'],
			                          'main_co' => $env['coordinator']
			                          )) // what ever....
			  )
			{
				$return[] = 'add_project';
