...
*/
		...
		function read_single($project_id = 0, $values = array(), $view = false)
		{
			...
			for ($i=0;$i<count($workorder_data);$i++)
			{
				...
				$values['workorder_budget'][$i]['contract_sum']=number_format($workorder_data[$i]['contract_sum'] * (1+(((int)$workorder_data[$i]['addition_percentage'])/100)), 2, ',', '');
				...
				$values['workorder_budget'][$i]['actual_cost'] = $workorder_data[$i]['actual_cost'] ? $workorder_data[$i]['actual_cost'] : 0;
