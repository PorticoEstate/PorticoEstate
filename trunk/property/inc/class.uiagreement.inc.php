	...
	*/
	...
	class property_uiagreement
	{
		...
		function property_uiagreement()
		{
			...
			$this->bo		= CreateObject($this->currentapp.'.boagreement',True);
...
//html_print_r($content);
		...
		{
			$agreement_id	= get_var('agreement_id',array('POST','GET'));
...
//						$values['agreement_id']	= $id;
				...
				elseif(($values['enable_alarm'] || $values['disable_alarm']) && count($values['alarm'])):
				{
					...
					if(!$receipt['error'])
					{
						$receipt = $boalarm->enable_alarm('agreement',$values['alarm'],$values['enable_alarm']);
					}
...
//				$table_header=$list['table_header'];
			...
			(
				...
				'lookup_functions'			=> $agreement['lookup_functions'],
