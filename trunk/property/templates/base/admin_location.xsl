									...
									<xsl:when test="selected='selected'">
										<input type="radio" name="values[{//column_name}]" value="{id}" checked="checked" onMouseout="window.status='';return true;">
											<xsl:attribute name="onMouseover">
												...
												<xsl:value-of select="//lang_config_statustext"></xsl:value-of>
				...
				<form method="post" action="{$form_action}">
					...
					<xsl:choose>
						<xsl:when test="value_id != ''">
							<tr>
								<td valign="top">
									<xsl:value-of select="lang_list_info"></xsl:value-of>
								</td>
								<td align="right">
									<xsl:call-template name="list_info"></xsl:call-template>
								</td>
							</tr>
							<tr>
								<td align="left">
									<xsl:value-of select="php:function('lang', 'list address')"></xsl:value-of>							
								</td>
								<td>
									<input type="checkbox" name="values[list_address]" value="1">
										<xsl:attribute name="title">
											<xsl:value-of select="php:function('lang', 'list address at this level')"></xsl:value-of>
										</xsl:attribute>
							...
							<tr>
								<td align="left">
									<xsl:value-of select="php:function('lang', 'list documents')"></xsl:value-of>							
								</td>
								<td>
									<input type="checkbox" name="values[list_documents]" value="1">
										<xsl:attribute name="title">
											<xsl:value-of select="php:function('lang', 'list documents at level')"></xsl:value-of>
										</xsl:attribute>
		...
		<table width="100%" cellpadding="2" cellspacing="2" align="center">
			<tr>
				<td align="left">
					...
					<xsl:text>: </xsl:text>
					<xsl:value-of select="value_type_name"></xsl:value-of>
				</td>
			</tr>
			<tr>
				<td align="right">
		...
		<div align="left">
			...
			<table cellpadding="2" cellspacing="2" width="80%" align="center">
				...
				<form method="post" action="{$form_action}">
					...
					<tr>
						<td valign="top">
							<xsl:value-of select="lang_location_type"></xsl:value-of>
						</td>
						<td valign="top">
							<xsl:variable name="lang_entity_statustext"><xsl:value-of select="lang_entity_statustext"></xsl:value-of></xsl:variable>
							<xsl:variable name="select_location_type"><xsl:value-of select="select_location_type"></xsl:value-of></xsl:variable>
							<select name="{$select_location_type}" class="forms" title="{$lang_entity_statustext}">
								<option value=""><xsl:value-of select="lang_no_location_type"></xsl:value-of></option>
								<xsl:apply-templates select="entity_list"></xsl:apply-templates>
							</select>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td>
							<xsl:value-of select="php:function('lang', 'show in lookup forms')"></xsl:value-of>
						</td>
						<td>
							<input type="checkbox" name="values[lookup_form]" value="1">
								...
								<xsl:attribute name="title">
									<xsl:value-of select="php:function('lang', 'check to show this attribue in lookup forms')"></xsl:value-of>
								</xsl:attribute>
							</input>
						</td>
					</tr>
						...
						<xsl:choose>
							<xsl:when test="selected='selected'">
								<input type="checkbox" name="values[list_info][{id}]" value="{id}" checked="checked" onMouseout="window.status='';return true;">
									<xsl:attribute name="onMouseover">
										<xsl:text>window.status='</xsl:text>
										<xsl:value-of select="//lang_list_type_statustext"></xsl:value-of>
										<xsl:text>'; return true;</xsl:text>
									</xsl:attribute>
								</input>
							</xsl:when>
