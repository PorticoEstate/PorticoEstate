<!-- $Id$ -->
	...
	<xsl:template name="alarm_data">
		<table width="80%" cellpadding="2" cellspacing="2" align="center">
			...
			<xsl:call-template name="add_alarm"></xsl:call-template>
		</table>
	</xsl:template>
	...
	<xsl:template match="add_alarm">
		<tr height="20">
			<td valign="top" align="right" colspan="5">
				<xsl:value-of select="lang_add_alarm"></xsl:value-of>
				<xsl:text> : </xsl:text>

				<xsl:variable name="lang_day_statustext"><xsl:value-of select="lang_day_statustext"></xsl:value-of></xsl:variable>
				<select name="values[time][days]" class="forms" onMouseover="window.status='{$lang_day_statustext}'; return true;" onMouseout="window.status='';return true;">
					...
					<xsl:apply-templates select="day_list"></xsl:apply-templates>
				</select>
				<xsl:value-of select="lang_day"></xsl:value-of>
				<xsl:variable name="lang_hour_statustext"><xsl:value-of select="lang_hour_statustext"></xsl:value-of></xsl:variable>
				<select name="values[time][hours]" class="forms" onMouseover="window.status='{$lang_hour_statustext}'; return true;" onMouseout="window.status='';return true;">
					<option value="">0</option>
					<xsl:apply-templates select="hour_list"></xsl:apply-templates>
				</select>
				<xsl:value-of select="lang_hour"></xsl:value-of>

				<xsl:variable name="lang_minute_statustext"><xsl:value-of select="lang_minute_statustext"></xsl:value-of></xsl:variable>
				<select name="values[time][mins]" class="forms" onMouseover="window.status='{$lang_minute_statustext}'; return true;" onMouseout="window.status='';return true;">
					<option value="">0</option>
					<xsl:apply-templates select="minute_list"></xsl:apply-templates>
				</select>
