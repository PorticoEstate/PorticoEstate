<!-- $Id$ -->
	...
	<xsl:template name="filter_member_of">
		...
		<xsl:variable name="member_of_name"><xsl:value-of select="member_of_name"></xsl:value-of></xsl:variable>
		...
		<form method="post" action="{$select_action}">
			<select name="{$member_of_name}" onChange="this.form.submit();" onMouseout="window.status='';return true;">
				...
				<option value=""><xsl:value-of select="lang_no_member"></xsl:value-of></option>
