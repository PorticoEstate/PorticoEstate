...
<xsl:template name="app_data">
	<xsl:choose>
		<xsl:when test="edit_type">
			<xsl:apply-templates select="edit_type"></xsl:apply-templates>
		</xsl:when>
		<xsl:when test="edit_contact">
			<xsl:apply-templates select="edit_contact"></xsl:apply-templates>
		</xsl:when>
		<xsl:when test="list_contact">
			<xsl:apply-templates select="list_contact"></xsl:apply-templates>
		</xsl:when>
		<xsl:otherwise>
			<xsl:apply-templates select="list_type"></xsl:apply-templates>
		</xsl:otherwise>
	</xsl:choose>
</xsl:template>
...
<xsl:template match="list_type">
	<xsl:variable name="responsible_action"><xsl:value-of select="responsible_action"></xsl:value-of></xsl:variable>
	...
	<table width="100%" cellpadding="2" cellspacing="2" align="center">
		<xsl:apply-templates select="table_header_type"></xsl:apply-templates>
		<xsl:choose>
			<xsl:when test="values_type != ''">
				<xsl:apply-templates select="values_type"></xsl:apply-templates>
			</xsl:when>
		</xsl:choose>
		<xsl:choose>
...
<xsl:template match="table_header_type">
	...
	<tr class="th">
		...
		<td class="th_text" width="5%" align="center">
			<xsl:value-of select="lang_created_by"></xsl:value-of>
		</td>
		<td class="th_text" width="5%" align="center">
			<xsl:value-of select="lang_app_name"></xsl:value-of>
		</td>
		<td class="th_text" width="1%" align="center">
			<xsl:value-of select="lang_active"></xsl:value-of>
		</td>
		<td class="th_text" width="5%" align="center">
			<xsl:value-of select="lang_contacts"></xsl:value-of>
		</td>
		<td class="th_text" width="5%" align="center">
...
<xsl:template match="values_type">
	<tr>
		...
		<td align="left">
			<xsl:value-of select="app_name"></xsl:value-of>
		</td>
		<td align="center">
			<xsl:value-of select="active"></xsl:value-of>
		</td>
		<td align="center" title="{lang_contacts_text}">
			<xsl:variable name="link_contacts"><xsl:value-of select="link_contacts"></xsl:value-of></xsl:variable>
			<a href="{link_contacts}"><xsl:value-of select="text_contacts"></xsl:value-of></a>
		</td>
		<xsl:choose>
			<xsl:when test="link_edit != ''">
				<td align="center" title="{lang_edit_text}">
					...
					<a href="{link_edit}"><xsl:value-of select="text_edit"></xsl:value-of></a>
				</td>
			</xsl:when>
		</xsl:choose>
		<xsl:choose>
			<xsl:when test="link_delete != ''">
				<td align="center" title="{lang_delete_text}">
					...
					<a href="{link_delete}"><xsl:value-of select="text_delete"></xsl:value-of></a>
				</td>
			</xsl:when>
		</xsl:choose>
		<xsl:choose>
	...
	<table width="100%" cellpadding="2" cellspacing="2" align="center">
		...
		<tr>
			<td class="th_text" align="left">
				<xsl:value-of select="location_name"></xsl:value-of>
				<xsl:choose>
...
<xsl:template match="table_header_contact">
	<xsl:variable name="sort_location"><xsl:value-of select="sort_location"></xsl:value-of></xsl:variable>
	<xsl:variable name="sort_active_from"><xsl:value-of select="sort_active_from"></xsl:value-of></xsl:variable>
	<xsl:variable name="sort_active_to"><xsl:value-of select="sort_active_to"></xsl:value-of></xsl:variable>
	<tr class="th">
		<td class="th_text" width="20%" align="center">
			<xsl:value-of select="lang_contact"></xsl:value-of>
		</td>
		<td class="th_text" width="10%" align="left">
			<a href="{sort_ecodimb}"><xsl:value-of select="lang_ecodimb"></xsl:value-of></a>
		</td>
		<td class="th_text" width="10%" align="left">
			<a href="{$sort_location}"><xsl:value-of select="lang_location"></xsl:value-of></a>
		</td>
		<td class="th_text" width="5%" align="center">
			<xsl:value-of select="lang_item"></xsl:value-of>
		</td>
		<td class="th_text" width="5%" align="left">
			<a href="{$sort_active_from}"><xsl:value-of select="lang_active_from"></xsl:value-of></a>
		</td>
		<td class="th_text" width="5%" align="left">
			<a href="{$sort_active_to}"><xsl:value-of select="lang_active_to"></xsl:value-of></a>
		</td>
		<td class="th_text" width="5%" align="center">
			<xsl:value-of select="lang_created_on"></xsl:value-of>
		</td>
		<td class="th_text" width="5%" align="center">
			<xsl:value-of select="lang_created_by"></xsl:value-of>
		</td>
...
-->
	...
	<xsl:template match="values_contact">
		<tr>
			...
			<td align="left">
				<xsl:value-of select="ecodimb"></xsl:value-of>
			</td>
			<td align="left">
			...
			<td align="left">
				<xsl:value-of select="active_from"></xsl:value-of>
			</td>
			<td align="left">
				<xsl:value-of select="active_to"></xsl:value-of>
			</td>
			<td align="left">
							...
							<xsl:choose>
								<xsl:when test="value_active = '1'">
									<input type="checkbox" name="values[active]" value="1" checked="checked" onMouseout="window.status='';return true;">
										<xsl:attribute name="title">
											<xsl:value-of select="lang_active_on_statustext"></xsl:value-of>
										</xsl:attribute>
									</input>
								</xsl:when>
								<xsl:otherwise>
									<input type="checkbox" name="values[active]" value="1" onMouseout="window.status='';return true;">
										<xsl:attribute name="title">
											<xsl:value-of select="lang_active_off_statustext"></xsl:value-of>
										</xsl:attribute>
									</input>
								</xsl:otherwise>
							</xsl:choose>
								...
								<tr>
									<td valign="bottom">
										...
										<input type="submit" name="values[save]" value="{$lang_save}" onMouseout="window.status='';return true;">
											<xsl:attribute name="title">
												<xsl:value-of select="lang_save_status_text"></xsl:value-of>
											</xsl:attribute>
										</input>
									</td>
									<td valign="bottom">
										...
										<input type="submit" name="values[apply]" value="{$lang_apply}" onMouseout="window.status='';return true;">
											<xsl:attribute name="title">
												<xsl:value-of select="lang_apply_status_text"></xsl:value-of>
											</xsl:attribute>
										</input>
									</td>
									<td align="left" valign="bottom">
										...
										<input type="submit" name="values[cancel]" value="{$lang_cancel}" onMouseout="window.status='';return true;">
											<xsl:attribute name="title">
												<xsl:value-of select="lang_cancel_status_text"></xsl:value-of>
											</xsl:attribute>
										</input>
									</td>
								</tr>
		...
		<div align="left">
			...
			<form method="post" action="{$form_action}" name="form">
				<table cellpadding="2" cellspacing="2" width="80%" align="center">
					...
					<tr>
						<td>
							<xsl:value-of select="lang_location"></xsl:value-of>
						</td>
						<td>
							<xsl:value-of select="value_location_name"></xsl:value-of>
						</td>
					</tr>
					<xsl:choose>
						<xsl:when test="value_id != ''">
					...
					<tr>
						<td>
							<xsl:value-of select="lang_responsibility"></xsl:value-of>
						</td>
						<td>
							<input type="text" name="responsibility_id" value="{value_responsibility_id}" readonly="readonly" size="5" onMouseout="window.status='';return true;">
								<xsl:attribute name="title">
									<xsl:value-of select="lang_responsibility_status_text"></xsl:value-of>
								</xsl:attribute>
							</input>
					...
					<tr>
						...
						<td>
							<input type="text" name="contact" value="{value_contact_id}" onClick="lookup_contact()" readonly="readonly" size="5" onMouseout="window.status='';return true;">
								<xsl:attribute name="title">
									<xsl:value-of select="lang_contact_status_text"></xsl:value-of>
								</xsl:attribute>
							</input>
					...
					<tr>
						<td>
							<xsl:value-of select="lang_active_from"></xsl:value-of>
						</td>
						<td>
							<input type="text" id="values_active_from" name="values[active_from]" size="10" value="{value_active_from}" readonly="readonly" onMouseout="window.status='';return true;">
								<xsl:attribute name="title">
									<xsl:value-of select="lang_active_from_statustext"></xsl:value-of>
								</xsl:attribute>
							</input>
							<img id="values_active_from-trigger" src="{img_cal}" alt="{lang_datetitle}" title="{lang_datetitle}" style="cursor:pointer; cursor:hand;"></img>
						</td>
					</tr>
					<tr>
						<td>
							<xsl:value-of select="lang_active_to"></xsl:value-of>
						</td>
						<td>
							<input type="text" id="values_active_to" name="values[active_to]" size="10" value="{value_active_to}" readonly="readonly" onMouseout="window.status='';return true;">
								<xsl:attribute name="title">
									<xsl:value-of select="lang_active_to_statustext"></xsl:value-of>
								</xsl:attribute>
							</input>
							<img id="values_active_to-trigger" src="{img_cal}" alt="{lang_datetitle}" title="{lang_datetitle}" style="cursor:pointer; cursor:hand;"></img>
						</td>
					</tr>
