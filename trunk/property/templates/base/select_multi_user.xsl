<!-- $Id$ -->
	...
	<xsl:template name="select_multi_user">
		...
		<xsl:variable name="select_name_user"><xsl:value-of select="select_name_user"/></xsl:variable>
	...
	<xsl:template match="user_list">
		<xsl:variable name="account_id"><xsl:value-of select="account_id"/></xsl:variable>
		<xsl:choose>
			<xsl:when test="selected">
				<option value="{$account_id}" selected="selected"><xsl:value-of disable-output-escaping="yes" select="account_lid"/></option>
			</xsl:when>
			<xsl:otherwise>
				<option value="{$account_id}"><xsl:value-of disable-output-escaping="yes" select="account_lid"/></option>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
