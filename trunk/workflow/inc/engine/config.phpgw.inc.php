...
/**
 * Configuration of the Galaxia Workflow Engine for phpGroupWare
 */

// Common prefix used for all database table names, e.g. galaxia_
if (!defined('GALAXIA_TABLE_PREFIX')) {
		define('GALAXIA_TABLE_PREFIX', 'phpgw_wf_');
}
...
if (!defined('GALAXIA_PROCESSES'))
{
		...
		define('GALAXIA_PROCESSES', $GLOBALS['phpgw']->vfs->basedir.'/'.'workflow');
}
...
if (!defined('GALAXIA_LOGFILE')) {
		...
		define('GALAXIA_LOGFILE',  $GLOBALS['phpgw']->vfs->basedir.'/'.'workflow'.'/'.'galaxia.log');
}
		...
		*/
		function galaxia_retrieve_user_groups($user=0)
		{
			...
			else
			{
				...
				// in phpgroupware we retrieve the already loaded in memory group list.
...
if (!function_exists('galaxia_get_config_values'))
{
	function galaxia_get_config_values($parameters=array())
	{
		...
		//boolean warning: phpgw'config class is not storing false values if it is 0
