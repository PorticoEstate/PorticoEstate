<?php
	/**************************************************************************\
	* phpGroupWare Workflow - Mail SMTP Agent Connector - business layer         *
	* ------------------------------------------------------------------------ *
	...
	* any later version.                                                       *
	...
	/* $Id: class.bo_agent_mail_smtp.inc.php 20178 2005-12-23 20:36:40Z regis_glc $ */


	/**
	 *  * Mail-SMTP Agent : business layer
	 *  *
	 *  * This class connects the workflow agent to the phpgroupware phpmailer and emailadmin
	...
	require_once(dirname(__FILE__) . '/' . 'class.bo_agent.inc.php');
	...
	class bo_agent_mail_smtp extends bo_agent
	{
		...
		var $mail = null;
		//the emailadmin bo object to retriev phpgroupware mail configuration
		var $bo_emailadmin = null;
		//the emailadmin profile id
		var $profileID;
		// some maybe usefull phpgroupware or engine objects. Vars usefull to create only the first time
		...
		function bo_agent_mail_smtp()
		{
			...
			$this->ProcessConfigurationFieldsdefault = array(
				...
				'mail_smtp_signature'		=> lang('Mail automatically sent by Mail SMTP Agent for phpGroupWare\'s Workflow'),
			...
			$this->help .= lang('Link addresses are considered local if not containing <strong>http://</strong>. They will get appended the configured local prefix and scanned by phpgroupware link engine');
			$this->help .= "</ul>\n";
			$this->fields = array(
				'wf_to'		=> array(
					'type'		=> 'text',
		...
		*  * @return an array which can be empty
		...
		/**
		 * * return the SMTP config values stored by the emailadmin phpgw application
		...
		*  * @return true if everything was ok, false if something went wrong
		...
		function prepare_mail()
		{
			...
			$this->mail->AddCustomHeader("X-Mailer: phpGroupWare Workflow");
