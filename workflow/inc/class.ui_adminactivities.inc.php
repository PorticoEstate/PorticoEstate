<?php
	...
	include(dirname(__FILE__) . '/' . 'class.workflow.inc.php');
	...
	class ui_adminactivities extends workflow
	{
		...
		*  * @return new role id
		...
		{
			$proc_info = $this->process_manager->get_process($this->wf_p_id);
			$image_name = $proc_info['wf_normalized_name'] . '/' . 'graph' . '/' . $proc_info['wf_normalized_name'] . '.png';
			$image = GALAXIA_PROCESSES . '/' . $image_name;
			//die($image);
