...
class ProcessManager extends BaseManager {
  ...
  */
  ...
  function new_process_version($pId, $minor=true)
  {
    ...
    $this->_rec_copy(GALAXIA_PROCESSES.'/'.$oldname.'/'.'code',GALAXIA_PROCESSES.'/'.$newname.'/'.'code');
  ...
  */
  function _create_directory_structure($name)
  {
    ...
    $path = GALAXIA_PROCESSES.'/'.$name;
    if (!file_exists($path)) mkdir($path,0770);
    $path = GALAXIA_PROCESSES.'/'.$name.'/'."graph";
    if (!file_exists($path)) mkdir($path,0770);
    $path = GALAXIA_PROCESSES.'/'.$name.'/'."code";
    if (!file_exists($path)) mkdir($path,0770);
    $path = GALAXIA_PROCESSES.'/'.$name.'/'."compiled";
    if (!file_exists($path)) mkdir($path,0770);
    $path = GALAXIA_PROCESSES.'/'.$name.'/'."code".'/'."activities";
    if (!file_exists($path)) mkdir($path,0770);
    $path = GALAXIA_PROCESSES.'/'.$name.'/'."code".'/'."templates";
    if (!file_exists($path)) mkdir($path,0770);
    if (GALAXIA_TEMPLATES) {
      $path = GALAXIA_TEMPLATES.'/'.$name;
      if (!file_exists($path)) mkdir($path,0770);
    }
    // Create shared file
    $file = GALAXIA_PROCESSES.'/'.$name.'/'."code".'/'."shared.php";
    if (!file_exists($file))
    {
